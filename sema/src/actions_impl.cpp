...
{
  if (!statements.empty() && is_a<rule_invocation>(statements.front()) &&
      as<rule_invocation>(statements.front())->name() == "project") {
    const ast::statements_t st_without_project_def(
      statements.begin() + 1, statements.end(), statements.get_allocator());
    return new (ctx_) ast::hamfile(as<rule_invocation>(statements.front()),
                                   st_without_project_def);
  } else
...
static int
required_argument_count(const rule_declaration& rule_decl)
{
  int result = 0;
  for (rule_declaration::const_iterator i = rule_decl.begin(),
                                        last = rule_decl.end();
       i != last;
       ++i)
    if (!i->is_optional())
                       ...
                       context& ctx)
{
  if (const list_of* list = as<list_of>(arg))
    if (list->values().size() == 1)
      if (const ast::path_like_seq* pls =
            as<ast::path_like_seq>(list->values().front()))
        if (pls->is_simple())
          return new (ctx) id_expr(pls->to_identifier());

  ctx.diag_.error(arg->start_loc(), "Argument '%s': must be simple identifier")
    << ra.name();
...
static const expression*
process_sources_decl_arg(const rule_argument& ra,
                         const expression* arg,
                         context& ctx)
{
                          ...
                          context& ctx)
{
  ...
  ctx.diag_.error(arg->start_loc(), "Argument '%s': must be path like sequence")
    << ra.name();
  return new (ctx) error_expression(arg);
}
                ...
                ast::context& ctx)
{
  if (!ra.is_optional() && is_a<empty_expr>(arg)) {
    ctx.diag_.error(arg->start_loc(),
                    "Required argument '%s' expected before '%s'")
      << ra.name() << as<empty_expr>(arg)->next_token();
                  ...
                  ast::context& ctx)
{
  ...
  for (expressions_t::const_iterator i = arguments.begin(),
                                     last = arguments.end();
       i != last;
       ++i) {
    if (is_a<named_expr>(**i)) {
      ...
      if (r == rule_decl.end()) {
        ctx.diag_.error(arg_name.start_lok(),
                        "%s '%s' does not have named argument '%s'")
      ...
      } else // named argument founded
      {
        if (used_named_args.find(&*r) != used_named_args.end()) {
          ctx.diag_.error(arg_name.start_lok(),
                          "%s '%s': named argument '%s' used more than once")
            << (rule_decl.is_target() ? "Target" : "Rule") << rule_name
            << arg_name;
  ...
  if (i != ctx_.rule_manager_.end()) {
    if (!first_rule_in_file_) {
      if (rule_name == "project")
        ctx_.diag_.error(
          rule_name.start_lok(),
          "Rule 'project' MUST be the first statement in the file");
    } else
      first_rule_in_file_ = false;

    return new (ctx_) ast::rule_invocation(
      rule_name, process_arguments(rule_name, i->second, arguments, ctx_));
  } else {
    ctx_.diag_.error(rule_name.start_lok(),
                     "Target or rule '%s' was not defined")
      << rule_name;
...
const ast::requirement*
actions_impl::on_conditional_requirement(
  parscore::source_location public_tag_loc,
  const ast::features_t& features,
  const ast::feature* value) const
{
  return new (ctx_)
    ast::conditional_requirement(public_tag_loc, features, value);
}
...
{
  return new (ctx_)
    ast::target_ref(public_tag, head, target_name, requirements);
}
}
}
