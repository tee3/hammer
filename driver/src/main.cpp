...
void
already_interrupted_ctrl_handler(int sig)
{
...
options_for_help()
{
  ...
  desc.add_options()("help", "produce this help message")(
    ...
    "up-to-date-check,c",
    po::bool_switch(&opts.only_up_to_date_check_),
    "instantiate/materialize + generate targets + up to date check")(
    "dump-targets-to-update",
    po::bool_switch(&opts.dump_targets_to_update_),
    "dump tree of target to update for debuginf purposes")(
    "clean-all",
    po::bool_switch(&opts.clean_all_),
    "clean all targets recursively")("generate-msvc-8.0-solution,p",
                                     "generate msvc-8.0 solution+projects")(
    "generate-projects-locally,l",
    po::bool_switch(&opts.generate_projects_localy_),
    "when generating build script makes them in one place")(
    "hammer-out",
    po::value<std::string>(&opts.hammer_output_dir_),
    "specify where hammer will place all its generated output")(
    "debug,d", po::value<int>(&opts.debug_level_), "specify verbosity level")(
    "disable-batcher",
    po::bool_switch(&opts.disable_batcher_),
    "do not build many sources at once")(
    "just-one-source,s",
    po::value<string>(&opts.just_one_source_),
    "build unconditionally specified source")(
    "just-one-source-project-path",
    po::value<string>(&opts.just_one_source_project_path_),
    "path to project where source reside")(
    "jobs,j", po::value<unsigned>(&opts.worker_count_), "concurrency level")(
    "copy-dependencies",
    po::bool_switch(&opts.copy_dependencies_),
    "copy shared modules to output dir when building excecutable")(
    "write-build-graph",
    po::bool_switch(&opts.write_build_graph_),
    "don't build, just write graphviz build-graph.dot for building process")(
    "update-warehouse",
    po::bool_switch(&opts.update_warehouse_),
    "update warehouse package database")(
    "update-all-warehouse-packages",
    po::bool_switch(&opts.update_all_warehouse_packages_),
    "update all warehouse packages that has been changed on the server")(
    "add-to-packages", "add current project into packages database")(
    "path-to-packages",
    po::value<std::string>(&opts.path_to_packages_),
    "path to packages database")(
    "release-package",
    po::bool_switch(&opts.release_package_),
    "add (release) package current to configured warehouse")(
...
feature*
try_resolve_implicit_feature(feature_registry& fr, const std::string& value)
{
  feature* result =
    try_resolve_implicit_feature(fr, fr.get_def("toolset"), value);
  if (result != NULL)
    return result;
...
void
resolve_arguments(vector<string>& targets,
                  feature_set* build_request,
                  feature_registry& fr,
                  const vector<string>& build_request_options)
{
  typedef vector<string>::const_iterator iter;
  for (iter i = build_request_options.begin(),
            last = build_request_options.end();
       i != last;
       ++i) {
...
void
add_all_targets(vector<string>& targets, const hammer::project& project)
{
  for (hammer::project::targets_t::const_iterator
         i = project.targets().begin(),
         last = project.targets().end();
       i != last;
       ++i) {
...
bool
is_looks_like_project(const string& s)
{
  return s.find('/') != string::npos;
}

void
split_target_path(string& target_path,
                  string& target_name,
...
bool
project_has_multiple_targets(const hammer::project& p, const string& name)
{
  unsigned count = 0;
  for (hammer::project::targets_t::const_iterator i = p.targets().begin(),
                                                  last = p.targets().end();
       i != last;
       ++i)
    if (i->first == name)
                    ...
                    const feature_set& build_request)
{
  ...
  feature_set* usage_requirements =
    project.get_engine()->feature_registry().make_set();
  const feature_set* build_request_with_defs =
    project.get_engine()->feature_registry().add_defaults(
      build_request.clone());
  for (vector<string>::const_iterator i = targets.begin(), last = targets.end();
       i != last;
       ++i) {
    if (is_looks_like_project(*i)) {
      ...
      const hammer::engine::loaded_projects_t& p =
        project.get_engine()->load_project(target_path, project);
      selected_targets_t st =
        target_name.empty()
          ? p.select_best_alternative(*build_request_with_defs)
          : selected_targets_t(
              1,
              p.select_best_alternative(target_name, *build_request_with_defs));
      feature_set* usage_requirements =
        project.get_engine()->feature_registry().make_set();
      for (selected_targets_t::const_iterator t = st.begin(), t_last = st.end();
           t != t_last;
           ++t) {
        t->target_->instantiate(
          NULL, build_request, &result, usage_requirements);
        usage_requirements->clear();
      }
    } else {
      const string& name = *i;
      if (project_has_multiple_targets(project, *i)) {
        hammer::project::selected_target target =
          project.select_best_alternative(name, *build_request_with_defs);
        target.target_->instantiate(
          nullptr, build_request, &result, usage_requirements);
...
nodes_t
generate_targets(const vector<basic_target*>& targets)
{
  typedef vector<basic_target*> targets_t;
  nodes_t result;
  for (targets_t::const_iterator i = targets.begin(), last = targets.end();
       i != last;
       ++i) {
...
void
generate_msvc80_solution(const nodes_t& nodes,
                         const hammer::project& project_to_build)
{
  project_generators::msvc_solution solution(
    project_to_build,
...
void
remove_propagated_targets(nodes_t& nodes, const project& project)
{
...
void
find_top_source_project_nodes(const build_node& node,
                              nodes_t& result,
                              ...
                              const project* source_project)
{
  ...
  for (nodes_t::const_iterator i = node.down_.begin(), last = node.down_.end();
       i != last;
       ++i)
...
nodes_t
find_top_source_project_nodes(const nodes_t& nodes,
                              const project* source_project)
{
  visited_nodes_t visited_nodes;
  nodes_t result;
...
bool
is_suffix_of(const string& suffix, const string& s)
{
...
bool
find_node_for_source_name(build_node_ptr& result,
                          visited_nodes_t& visited_nodes,
                          ...
                          const project* source_project)
{
  ...
  for (nodes_t::const_iterator i = node->down_.begin(),
                               last = node->down_.end();
       i != last;
       ++i)
    if (find_node_for_source_name(
          result, visited_nodes, *i, source_name, source_project)) {
      if (!result) {
...
build_node_ptr
find_nodes_for_source_name(const nodes_t& nodes,
                           const string& source_name,
                           const project* source_project)
{
  visited_nodes_t visited_nodes;
...
void
mark_nodes_to_update(nodes_t& nodes)
{
...
void
collect_nodes_for_one_source(visited_nodes_t& visited_nodes,
                             ...
                             const build_node& source_node)
{
  ...
  for (build_node_ptr& n : node.down_) {
    if (n.get() == &source_node && !path.empty()) {
      ...
      collect_nodes_for_one_source(
        visited_nodes, nodes_to_build, path, *n, source_node);
  }
...
void
run_build(nodes_t& nodes, engine& e, hammer_options opts)
{
  ...
  if (opts.just_one_source_.empty()) {
    ...
    builder builder(
      build_environment, interrupt_flag, opts.worker_count_, false);
    ...
    if (build_result.failed_to_build_targets_)
      cout << "...failed updating " << build_result.failed_to_build_targets_
           << " targets...\n";
    ...
    // FIXME: we must resolve symlinks because under linux fs::current_path give
    // as fully resolved path without any symlinks.
    // That is really wierd I think, but can't find any other solution.
    if (!opts.just_one_source_project_path_.empty())
      project_for_source =
        &e.load_project(resolve_symlinks(opts.just_one_source_project_path_));

    build_node_ptr source_node = find_nodes_for_source_name(
      nodes, opts.just_one_source_, project_for_source);
    if (!source_node)
      throw std::runtime_error("Source file not founded");

    nodes_t source_project_nodes =
      find_top_source_project_nodes(nodes, project_for_source);

    // collect all nodes prior to main source project nodes
    visited_nodes_t visited_nodes;
    nodes_t nodes_to_build;
    nodes_t path;
    for (nodes_t::iterator i = source_project_nodes.begin(),
                           last = source_project_nodes.end();
         i != last;
         ++i)
      collect_nodes_for_one_source(
        visited_nodes, nodes_to_build, path, **i, *source_node);
    ...
    /*
         ...
         cout << "...building source '" << opts.just_one_source_ << "'..."<<
   endl;
         builder::result build_result = builder.build(nodes_to_build,
   project_for_source);
         cout << "...source '" << opts.just_one_source_ << "' builded."<<
   endl;
*/
    // remove dups
    std::sort(nodes_to_build.begin(), nodes_to_build.end());
    nodes_to_build.erase(
      std::unique(nodes_to_build.begin(), nodes_to_build.end()),
      nodes_to_build.end());
                 ...
                 string* toolset_home_)
{
  ...
  e.toolset_manager().init_toolset(e,
                                   toolset_name,
                                   toolset_version,
                                   toolset_home_ == NULL ? NULL
                                                         : &toolset_home);
}

bool
has_configured_toolsets(const hammer::engine& e)
{
  const feature_registry& fs = e.feature_registry();
  const feature_def* toolset_def = fs.find_def("toolset");
  if (!toolset_def)
    throw std::runtime_error(
      "Internal error: Feature 'toolset' not registered");

  return !toolset_def->legal_values().empty();
}

void
autoconfigure_toolsets(hammer::engine& e)
{
  e.toolset_manager().autoconfigure(e);
}

void
do_clean_all(nodes_t& nodes, engine& e)
{
  cout << "...cleaning...";
  build_environment_impl build_environment(fs::current_path());
...
public:
  ...
  virtual const location_t& location() const { return destination_; }
...
void
add_copy_dependencies_nodes(nodes_t& nodes, engine& e)
{
  ...
  if (executables.size() != 1)
    throw std::runtime_error(
      "Dependency copying working only with one exe target");
  ...
  const generator& copy_generator =
    *e.generators()
  ...
  nodes_t copy_nodes;
  for (build_node::sources_t::const_iterator i = shared_libs.begin(),
                                             last = shared_libs.end();
       i != last;
       ++i) {
    boost::intrusive_ptr<build_node> new_node(
      new build_node(executables[0].source_node_->products_owner(), false));
    ...
    dep_copy_target* new_target =
      new dep_copy_target(executables[0].source_target_->location(),
...
string
human_readable_byte_count(const long long bytes, const bool si = true)
{
  ...
  return (boost::format("%.1f %c%sB") % (bytes / pow(unit, exp)) %
          ((si ? "kMGTPE" : "KMGTPE")[exp - 1]) % (si ? "" : "i"))
    .str();
}
...
{
public:
  warehouse_dl_notifier(const unsigned max_package_name_lenght)
    : max_package_name_lenght_(max_package_name_lenght)
  {
  }

  void on_download_begin(const warehouse::package_info& package) override
  {
    cout << "Downloading " << setw(max_package_name_lenght_) << left
         << (package.name_ + " (" + package.version_ + ")") << setw(10) << right
         << human_readable_byte_count(package.package_file_size_) << flush;
  }
  ...
  {
    cout << "Installing  " << setw(max_package_name_lenght_) << left
         << (package.name_ + " (" + package.version_ + ")") << setw(10) << ' '
         << flush;
  }
...
main(int argc, char** argv)
{
  try {
    ...
    po::parsed_options options = po::command_line_parser(argc, argv)
                                   .options(desc)
                                   .positional(build_request_options)
                                   .run();
    po::store(options, vm);
    po::notify(vm);
    ...
    if (vm.count("add-to-packages")) {
      ...
      if (!vm.count("path-to-packages"))
        throw std::runtime_error(
          "You should specify path-to-packages parameter");
    }
    ...
    while (true) {
      ...
      types::register_standart_types(engine.get_type_registry(),
                                     engine.feature_registry());
      ...
      engine.generators().insert(
        std::auto_ptr<generator>(new copy_generator(engine)));
      engine.generators().insert(
        std::auto_ptr<generator>(new obj_generator(engine)));
      ...
      engine.scanner_manager().register_scanner(
        engine.get_type_registry().get(types::CPP), c_scaner);
      engine.scanner_manager().register_scanner(
        engine.get_type_registry().get(types::C), c_scaner);
      engine.scanner_manager().register_scanner(
        engine.get_type_registry().get(types::RC), c_scaner);
      ...
      engine.call_resolver().insert(
        "use-toolset",
        boost::function<void(project*, string&, string&, string*)>(
          boost::bind(use_toolset_rule, _1, boost::ref(engine), _2, _3, _4)));
      ...
      if (user_config_script.empty() || !exists(user_config_script)) {
        ...
        if (opts.debug_level_ > 0)
          cout << "...Loading user-config.ham at '"
               << user_config_script.string<string>() << "'..." << flush;
      ...
      if (opts.update_warehouse_) {
        warehouse::package_infos_t packages_needs_to_be_updated =
        ...
        for (const auto& package : packages_needs_to_be_updated)
          cout << package.name_ << "(" << package.version_
               << ") size: " << package.package_file_size_ << "\n";
      ...
      if (!has_configured_toolsets(engine)) {
        cerr << "WARNING!!!WARNING!!!WARNING!!!WARNING!!!\n"
                "No toolsets are configured and no one toolset founded by "
                "auto-configure!\n"
                "Please, specify some toolset in $(HOME)/user-config.ham to "
                "operate properly.\n"
      ...
      if (vm.count("build-request"))
        resolve_arguments(targets,
                          build_request,
                          engine.feature_registry(),
                          vm["build-request"].as<vector<string>>());
...
// lets handle 'toolset' feature in build request
      ...
      if (i_toolset_in_build_request == build_request->end()) {
        const feature_def& toolset_definition =
          engine.feature_registry().get_def("toolset");
        if (!toolset_definition.is_legal_value(default_toolset_name))
          throw std::runtime_error(
            "Default toolset is set to '" + default_toolset_name +
            "', but either you didn't configure it in user-config.ham or it "
            "has failed to autoconfigure");

        const subfeature_def& toolset_version_def =
          toolset_definition.get_subfeature("version");
        if (toolset_version_def.legal_values(default_toolset_name).size() == 1)
          build_request->join(
            "toolset",
            (default_toolset_name + "-" +
             *toolset_version_def.legal_values(default_toolset_name).begin())
              .c_str());
        else
          throw std::runtime_error(
            "Default toolset is set to '" + default_toolset_name +
            "', but has multiple version configured. You should request "
            "specific version to use.");
      } else {
        const feature& used_toolset = **i_toolset_in_build_request;
        if (!used_toolset.find_subfeature("version")) {
          const subfeature_def& toolset_version_def =
            used_toolset.definition().get_subfeature("version");
          if (toolset_version_def.legal_values(used_toolset.value()).size() > 1)
            throw std::runtime_error(
              "Toolset is set to '" + used_toolset.value() +
              "', but has multiple version configured. You should request "
              "specific version to use.");
          else {
            ...
            build_request->join(
              "toolset",
              (toolset + "-" +
               *toolset_version_def.legal_values(toolset).begin())
                .c_str());
          }
        }
      }
      ...
      if (build_request->find("host-os") == build_request->end())
        build_request->join(
          "host-os",
          engine.feature_registry().get_def("host-os").get_default().c_str());
      ...
      if (opts.debug_level_ > 0) {
        cout << "...Targets to " << (opts.clean_all_ ? "clean-all" : "build")
             << " is: ";
        bool first_pass = true;
        for (vector<string>::const_iterator i = targets.begin(),
                                            last = targets.end();
             i != last;
             ++i) {
          if (first_pass)
      ...
      vector<basic_target*> instantiated_targets(
        instantiate_targets(targets, project_to_build, *build_request));
      cout << "Done." << endl;
      ...
      try {
        ...
        vector<warehouse::package_info> packages =
          wh.get_unresoved_targets_info(
            engine,
            find_all_warehouse_unresolved_targets(instantiated_targets));

        cout << boost::format("\n\nThere are %d unresolved package(s) to "
                              "download and install:\n\n") %
                  packages.size();

        auto package_with_longest_name =
          max_element(packages.begin(),
                      packages.end(),
                      [](const warehouse::package_info& lhs,
                         const warehouse::package_info& rhs) {
                        return lhs.name_.size() + lhs.version_.size() <
                               rhs.name_.size() + rhs.version_.size();
                      });
        const unsigned max_package_name_lenght =
          package_with_longest_name->name_.size() +
          package_with_longest_name->version_.size() + 3 /*formating extras*/;

        long long total_bytes_to_download = 0;
        for (vector<warehouse::package_info>::const_iterator
               i = packages.begin(),
               last = packages.end();
             i != last;
             ++i) {
          cout << setw(max_package_name_lenght) << left
               << (i->name_ + " (" + i->version_ + ")") << " : " << setw(10)
               << right << human_readable_byte_count(i->package_file_size_)
               << endl;
        ...
        cout << setw(max_package_name_lenght) << left
             ...
             << "  : " << setw(10) << right
             << human_readable_byte_count(total_bytes_to_download) << endl;
