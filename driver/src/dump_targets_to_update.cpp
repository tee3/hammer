...
static std::ostream&
put_indent(std::ostream& os, int indent)
{
  for (; indent > 0; --indent)
    os.put(' ');
     ...
     int indent)
{
  ...
  for (build_node::sources_t::const_iterator i = node.sources_.begin(),
    ...
    if (i->source_node_->up_to_date() != boost::tribool::true_value) {
      ...
      put_indent(os, indent)
        << "[s]" << i->source_target_->name() << " "
        << to_simple_string(i->source_target_->timestamp_info().timestamp_)
        << std::endl;
  ...
  for (build_node::targets_t::const_iterator i = node.products_.begin(),
       ...
       i != last;
       ++i)
    put_indent(os, indent) << "[p]" << (**i).name() << " "
                           << to_simple_string(
                                (**i).timestamp_info().timestamp_)
                           << std::endl;
}

void
dump_targets_to_update(std::ostream& os,
