...
{
  int rank = 0;
  for (feature_set::const_iterator i = constraints.begin(),
                                           ...
                                           bool full_match) const
{
  ...
  for (generators_t::const_iterator i = generators_.begin(),
                                    last = generators_.end();
       i != last;
       ++i) {
    ...
    for (generator::producable_types_t::const_iterator
           j = i->second->producable_types().begin(),
...
{
  for (generator::consumable_types_t::const_iterator
         i = current_generator.consumable_types().begin(),
...
generator_registry::construct(const main_target* mt) const
{
  ...
  for (main_target::sources_t::const_iterator i = mt->sources().begin(),
                                              last = mt->sources().end();
       i != last;
       ++i) {
  ...
  for (build_node::nodes_t::const_iterator s = generated_sources.begin(),
       ...
       s != s_last;
       ++s)
    for (main_viable_generators_t::iterator i = main_viable_generators.begin(),
                                            last = main_viable_generators.end();
         i != last;
         ++i)
      i->all_consumed_ =
        i->all_consumed_ && transform_to_consumable(*i->generator_,
                                                    *i->generator_,
                                                    *s,
                                                    &i->transformed_sources_,
                                                    mt->properties(),
                                                    *mt);
