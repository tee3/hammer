...
void
basic_meta_target::sources(const sources_decl& s)
{
  sources_ = s;
}

void
basic_meta_target::add_sources(const sources_decl& s)
{
  sources_.insert(s);
}
...
void
instantiate_meta_targets(const meta_targets_t& targets,
                         const feature_set& build_request,
                         const main_target* owner,
...
void
basic_meta_target::split_one_source(sources_decl* simple_targets,
...
void
basic_meta_target::split_sources(sources_decl* simple_targets,
                                 meta_targets_t* meta_targets,
  ...
  meta_targets_t* meta_targets) const
{
  ...
  // source has target_name_ only when it was explicitly requested (./foo//bar)
  // where target_name_ == "bar"
  engine::loaded_projects_t suitable_projects =
    get_engine()->load_project(source.target_path(), *project_);
  if (source.target_name().empty()) {
    try {
      hammer::project::selected_targets_t selected_targets(
        suitable_projects.select_best_alternative(
          *build_request_with_source_properties));
      for (hammer::project::selected_targets_t::const_iterator
    ...
    project::selected_target selected_target =
      suitable_projects.select_best_alternative(
        source.target_name(), *build_request_with_source_properties);
    meta_targets->push_back({ selected_target.target_, source.properties() });
  }
}
...
void
basic_meta_target::instantiate(const main_target* owner,
                               const feature_set& build_request,
                               std::vector<basic_target*>* result,
                               feature_set* usage_requirements) const
{
  if (is_cachable(owner)) {
    for (instantiation_cache_t::const_iterator
    ...
    result->insert(result->end(),
                   cache_item.instantiated_targets_.begin(),
                   cache_item.instantiated_targets_.end());
...
void
adjust_dependency_features_sources(feature_set& set_to_adjust,
                                   const basic_meta_target& relative_to_target)
{
  for (feature* f : set_to_adjust) {
    ...
    if (source.type() == nullptr /*source is meta-target*/ &&
        !source.target_path_is_global() && !has_slash(source.target_path())) {
