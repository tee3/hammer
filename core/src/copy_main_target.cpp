...
copy_main_target::copy_main_target(const basic_meta_target* mt,
  ...
  , recursive_(false)
{
  auto d = props->find("destination");
  if (d == props->end()) {
    throw std::runtime_error(
      "Copy main target must have <destination> feature in requirements");
  }
  ...
  for (auto i = props->find("type-to-copy"), last = props->end(); i != last;
       i = props->find(i + 1, "type-to-copy")) {
    types_to_copy_.push_back(&tr.get(type_tag((**i).value())));
  }

  if (types_to_copy_.empty()) {
    throw std::runtime_error("Copy main target must have at least one "
                             "<type-to-copy> feature in requirements");
  }

  // FIXME: By default recursive is on. Because recursive feature is optional,
  // we must turn it on by hands
  // Solution will be specify on what target types feature a works
  // feature.feature recursive : on off : : COPIED ;
  auto i = props->find("recursive");
...
void
copy_main_target::add_additional_dependencies(
  hammer::build_node& generated_node) const
{
  boost::intrusive_ptr<hammer::build_node> int_dir_node(
