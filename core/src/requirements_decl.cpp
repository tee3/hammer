...
requirements_decl::impl_t::clone() const
{
  ...
  for (requirements_t::const_iterator i = requirements_.begin(),
                                      last = requirements_.end();
       i != last;
       ++i)
...
void
requirements_decl::add(std::auto_ptr<requirement_base> r)
{
...
{
  std::auto_ptr<requirement_base> r(
                           ...
                           feature_set* public_result) const
{
  ...
  for (features_t::const_iterator i = features_.begin(), last = features_.end();
    ...
    if (build_request.contains(**i) != build_request.end() ||
      ...
      continue;
    } else if (build_request.find((**i).name()) != build_request.end() ||
...
{
  for (impl_t::iterator i = impl_->requirements_.begin(),
                        last = impl_->requirements_.end();
       i != last;
       ++i)
    i->setup_path_data(t);
