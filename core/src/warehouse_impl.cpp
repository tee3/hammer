...
struct warehouse_impl::gramma : public grammar<warehouse_impl::gramma>
{
  ...
  struct definition
  {
    ...
    explicit definition(const gramma& self)
    {
      ...
      value_p = lexeme_d[confix_p(
        ch_p('"'), (*anychar_p)[assign_a(self.value_)], ch_p('"'))];
      int_value_p = uint_p[boost::bind(&gramma::assign_int_value, &self, _1)];
      bool_value_p =
        str_p("true")[boost::bind(&gramma::assign_bool_value, &self, true)] |
      ...
      list_of_target_names_p =
        list_p(value_p[push_back_a(self.targets_, self.value_)], ch_p(','));
      targets_p = str_p("targets") >> colon >>
                  confix_p(ch_p('['), list_of_target_names_p, ch_p(']'));
      dependency_attrs_p =
        public_id_p[assign_a(self.dependency_.public_id_, self.value_)] |
        version_p[assign_a(self.dependency_.version_, self.value_)];
      dependencies_list_p = list_p(dependency_attrs_p, ch_p(','));
      dependency_p = confix_p(
        ch_p('{'),
        dependencies_list_p[push_back_a(self.package_.dependencies_,
                                        self.dependency_)]
                           [assign_a(self.dependency_, self.empty_dependency_)],
        ch_p('}'));
      dependencies_p =
        str_p("dependencies") >> colon >>
        confix_p(ch_p('['), list_p(dependency_p, ch_p(',')), ch_p(']'));
      attribute_p =
        version_p[assign_a(self.package_.version_, self.value_)] |
        ...
        targets_p[assign_a(self.package_.targets_, self.targets_)]
                 [assign_a(self.targets_, self.empty_targets_)] |
        dependencies_p;
      package_p =
        confix_p(ch_p('{'), list_p(attribute_p, ch_p(',')), ch_p('}'));
      packages_p =
        list_p(package_p[push_back_a(self.packages_, self.package_)]
                        [assign_a(self.package_, self.empty_package_)],
               ch_p(','));
...
static void
download_file(const fs::path& working_dir,
              const string& url,
              const string& to_file = string())
{
  if (url.find_first_of("file://") == 0) {
    fs::path source_file = url.substr(7);
    fs::path destination_file = to_file.empty()
                                  ? (working_dir / source_file.filename())
                                  : (working_dir / to_file);
...
static string
package_id_from_location(const location_t& project_path)
{
  if (!project_path.has_root_path())
    throw std::runtime_error("[package_id_from_location]: Bad project path '" +
                             project_path.string() + "'");
...
warehouse_impl::load_packages(const fs::path& filepath)
{
  ...
  copy(istreambuf_iterator<char>(f),
       ...
       back_inserter(packages_database_file));

  gramma g;
  if (!parse(packages_database_file.begin(),
             packages_database_file.end(),
             g,
             +space_p)
         .hit)
  ...
  packages_t packages;
  for (vector<package_t>::const_iterator i = g.packages_.begin(),
                                         last = g.packages_.end();
       i != last;
       ++i)
...
void
warehouse_impl::update_all_packages_impl(engine& e)
{
...
{
  pair<packages_t::iterator, packages_t::iterator> versions =
    packages.equal_range(public_id);
...
warehouse_impl::load_project(engine& e, const location_t& project_path)
{
  const string public_id = package_id_from_location(project_path);

  boost::shared_ptr<project> wproject(
    new warehouse_project(e, public_id, repository_path_ / "libs" / public_id));
...
warehouse_impl::project_from_warehouse(const project& p) const
{
  ...
  // return std::search(p.location().begin(), p.location().end(),
  // repository_path_.begin(), repository_path_.end()) == p.location().begin();
...
void
warehouse_impl::resolve_dependency(unresolved_packages_t& packages,
                                   engine& e,
                                   ...
                                   const project& repository_project) const
{
  ...
  engine::loaded_projects_t loaded_projects =
    e.try_load_project("/" + d.public_id_, repository_project);
  ...
  project::selected_targets_t targets =
    loaded_projects.select_best_alternative(*build_request);
  if (targets.size() != 1)
    throw std::runtime_error("Failed to resolve dependency for package " +
                             d.public_id_ + ":" + d.version_);

  if (!dynamic_cast<const warehouse_meta_target*>(targets.front().target_))
    return; // looks like package exists

  packages_t::const_iterator i_dependency_package =
    find_package(d.public_id_, d.version_);
  if (i_dependency_package == packages_.end())
    throw std::runtime_error("Package " + d.public_id_ + ":" + d.version_ +
                             " doesn't exist");

  const package_t& package = i_dependency_package->second;
  packages.insert(make_pair(dep_hash, to_package_info(package)));

  for (vector<dependency_t>::const_iterator i = package.dependencies_.begin(),
                                            last = package.dependencies_.end();
       i != last;
       ++i)
  ...
  const std::vector<const warehouse_target*>& /*targets*/) const
{
  ...
  for (std::vector<const warehouse_target*>::const_iterator
         i = targets.begin(),
         last = targets.end();
       i != last;
       ++i) {
    ...
    if (version_i != props.end())
      ...
      throw std::runtime_error(
        "Unable to get unresolved target info - no version specified");
    ...
    for (vector<dependency_t>::const_iterator
           i = package.dependencies_.begin(),
           last = package.dependencies_.end();
         i != last;
         ++i) {
  ...
  for (unresolved_dependencies_t::const_iterator i = deps.begin(),
                                                 last = deps.end();
       i != last;
       ++i)
  ...
  for (unresolved_packages_t::const_iterator i = packages.begin(),
                                             last = packages.end();
       i != last;
       ++i)
...
void
warehouse_impl::download_package(const package_t& p,
...
static void
insert_line_in_front(const fs::path& filename, const string& line)
{
  fs::path tmp_name =
    filename.branch_path() / (filename.filename().string() + ".tmp");
  {
    ...
    copy(istreambuf_iterator<char>(src),
         istreambuf_iterator<char>(),
         ostreambuf_iterator<char>(tmp_file));
  }
...
warehouse_impl::install_package(const package_t& p, const fs::path& working_dir)
{
  ...
  bp::child child =
    bp::launch_shell("bzcat " + package_archive.string() + " | tar -x", ctx);
  bp::status status = child.wait();
  if (status.exit_status() != 0)
  ...
  insert_line_in_front(
    package_hamfile,
    make_package_alias_line(p.public_id_, p.version_, p.targets_));
}

bool
warehouse_impl::resolves_to_real_project(engine& e,
  ...
  iwarehouse_download_and_install& /*notifier*/)
{
  ...
  for (std::vector<package_info>::const_iterator i = packages.begin(),
                                                 last = packages.end();
       i != last;
       ++i) {
    packages_t::const_iterator pi = find_package(i->name_, i->version_);
    if (pi == packages_.end())
      throw std::runtime_error("Can't find package '" + i->name_ + " v" +
                               i->version_ + "'");
    ...
    if (!resolves_to_real_project(
          e, pi->second.public_id_, repository_project)) {
      const fs::path repository_hamroot = repository_path_ / "hamroot";
      append_line(repository_hamroot,
                  "use-project /" + pi->second.public_id_ + " : ./libs/" +
                    pi->second.public_id_ + ";");
    }
...
void
warehouse_impl::write_packages(const location_t& packages_db_path,
                               const packages_t& packages)
{
  ...
  tmp_packages << "[\n";
  bool first = true;
  for (packages_t::const_iterator i = packages.begin(), last = packages.end();
       i != last;
       ++i) {
    if (first) {
      tmp_packages << "   {\n";
    ...
    tmp_packages << "       need_update: " << boolalpha
                 << i->second.need_update_ << ",\n";
    ...
    if (i->second.dependencies_.empty())
      ...
      bool d_first = true;
      for (vector<dependency_t>::const_iterator
             d = i->second.dependencies_.begin(),
             d_last = i->second.dependencies_.end();
           d != d_last;
           ++d) {
        if (d_first) {
          tmp_packages << "          {\n";
...
static void
make_package_archive(const fs::path& package_root,
                     const fs::path& package_filename)
{
  bp::context ctx;
  ctx.work_directory = package_root.string();
  ctx.stdout_behavior = bp::inherit_stream();
  bp::child child = bp::launch_shell("tar --exclude \\.git --exclude \\.svn "
                                     "--exclude \\.hammer -c . | bzip2 -c > " +
...
warehouse_impl::gather_dependencies(const project& p)
{
  ...
  for (project::targets_t::const_iterator i = p.targets().begin(),
                                          last = p.targets().end();
       i != last;
       ++i) {
    const sources_decl& sources = i->second->sources();
    for (sources_decl::const_iterator s_i = sources.begin(),
                                      s_last = sources.end();
         s_i != s_last;
         ++s_i) {
  ...
  source_dependencies.erase(
    unique(source_dependencies.begin(), source_dependencies.end()),
  ...
  for (const source_decl& s : source_dependencies) {
    ...
    if (i == s.properties()->end())
      throw std::runtime_error("Dependency '" + s.target_path() +
                               "' doesn't have version specified");
...
static string
extract_filepath_from_url(const string& url)
{
  if (url.find(file_schema) != 0)
    throw std::runtime_error(
      "Warehouse url'" + url +
      "' should be 'file://...' type to be able to add packages");
...
void
warehouse_impl::add_to_packages(const project& p,
                                const location_t& packages_db_root_)
{
  const location_t packages_db_root =
    packages_db_root_.empty() ? extract_filepath_from_url(repository_url_)
                              : packages_db_root_;
  ...
  if (!exists(packages_db_full_path)) {
    ...
    if (!f)
      throw std::runtime_error("Can't create '" +
                               packages_db_full_path.string() + "'");
    f << "[]";
  }
  ...
  feature_set* project_requirements =
    p.get_engine()->feature_registry().make_set();
  ...
  package.filename_ = boost::algorithm::replace_all_copy(public_id, "/", ".") +
                      "-" + version + ".tar.bz2";

  const fs::path package_full_path = packages_db_root / package.filename_;
  make_package_archive(p.location().branch_path().branch_path(),
                       package_full_path);
...
warehouse_impl::get_package_versions(const string& public_id) const
{
  ...
  for (; versions.first != versions.second; ++versions.first)
    result.push_back(
      { versions.first->second.version_, versions.first->second.targets_ });
                                  ...
                                  const fs::path& path_to_hamfile)
{
  ...
  const fs::path tmp_hamfile = path_to_hamfile.branch_path() /
                               (path_to_hamfile.filename().string() + ".tmp");
  fs::ofstream tmp_f(tmp_hamfile);
  fs::ifstream current_f(path_to_hamfile);

  string line;
  const string alias_line = make_package_alias_line(
    package_public_id, package_version, package_targets);
...
void
warehouse_impl::remove_package(const package_t& package_to_remove)
{
  ...
  remove_alias_from_package_hamfile(package_to_remove.public_id_,
                                    package_to_remove.version_,
                                    package_to_remove.targets_,
                                    lib_path / "hamfile");
...
void
warehouse_impl::update_package(engine& e, package_t& package_to_update)
{
