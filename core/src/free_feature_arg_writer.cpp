...
free_feature_arg_writer::free_feature_arg_writer(
  ...
  , global_suffix_(global_suffix)
{
}

void
free_feature_arg_writer::write_impl(std::ostream& output,
                                    const build_node& node,
                                    const build_environment& environment) const
{
  bool global_prefix_written = false;
  const feature_set& build_request = node.build_request();
  for (feature_set::const_iterator i = build_request.find(feature_def_.name()),
                                   last = build_request.end();
       i != last;) {
    ...
    if (feature_def_.attributes().path) {
      if (node.products_.empty())
        throw std::runtime_error("[free_feature_arg_writer] Can't write path "
                                 "feature for node without products.");

      location_t include_path((**i).get_path_data().target_->location() /
                              (**i).value());
      include_path.normalize();
      include_path = relative_path(
        include_path, environment.working_directory(*node.products_.front()));
      include_path.normalize();
...
free_feature_arg_writer*
free_feature_arg_writer::clone() const
{
