...
protected:
                    ...
                    const build_environment& environment) const override
  {
    ...
    product_argument_writer output_product_builder("compile-fail-output",
                                                   output_target_type_);
    ...
    env.write_tag_file(product_path_stream.str(),
                       env.output_stream_.str() + env.error_stream_.str());
...
compile_fail_generator::compile_fail_generator(
  engine& e,
  ...
  : generator(e,
              ...
              make_product_types(
                e,
                { types::TESTING_OUTPUT, types::TESTING_COMPILE_FAIL }),
              true,
              compile_generator->constraints())
  , compile_generator_(move(compile_generator))
{
  const target_type& output_type =
    e.get_type_registry().get(types::TESTING_OUTPUT);
  unique_ptr<compile_fail_build_action> compile_fail_action(
    new compile_fail_build_action(move(compile_action), output_type));
...
{
  build_nodes_t compile_nodes =
    compile_generator_->construct(type_to_construct,
                                  props,
                                  sources,
                                  source_target,
                                  composite_target_name,
                                  owner);
  // assume compile can produce only one node - can't imagine situations with
  // many, right now
  assert(compile_nodes.size() == 1);

  const build_nodes_t this_nodes = generator::construct(type_to_construct,
                                                        props,
                                                        sources,
                                                        source_target,
                                                        composite_target_name,
  ...
  compile_nodes.front()->products_.insert(
    compile_nodes.front()->products_.end(),
