...
bool
copy_action::execute_impl(const build_node& node,
                          const build_environment& environment) const
{
...
copy_generator::copy_generator(hammer::engine& e)
  : generator(
      ...
      true)
{
  std::unique_ptr<build_action> a(
    new copy_action(e.get_type_registry().get(types::COPIED)));
  action(std::move(a));
}
                          ...
                          const main_target& owner) const
{
  ...
  collect_nodes(collected_nodes,
                visited_nodes,
                sources,
                true_owner.types_to_copy(),
                true_owner.recursive());
