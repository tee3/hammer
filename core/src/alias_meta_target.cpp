...
alias_meta_target::alias_meta_target(hammer::project* p,
...
void
alias_meta_target::instantiate_impl(const main_target* owner,
                                    ...
                                    feature_set* usage_requirements) const
{
  if (owner != nullptr) {
    ...
    for (const source_decl& sd : sources()) {
      feature* f =
        get_engine()->feature_registry().create_feature("source", "");
