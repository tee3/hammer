...
generator::is_consumable(const target_type& t) const
{
  ...
  for (generator::consumable_types_t::const_iterator
         i = consumable_types().begin(),
         last = consumable_types().end();
       i != last;
       ++i)
                     ...
                     const main_target& owner) const
{
  if (!source_target) {
    ...
    for (iter i = sources.begin(), last = sources.end(); i != last; ++i) {
      ...
      for (build_node::targets_t::const_iterator p_i = (**i).products_.begin(),
                                                 p_last = (**i).products_.end();
           p_i != p_last;
           ++p_i) {
    ...
    unsigned p = 0;
    for (producable_types_t::const_iterator i = target_types_.begin(),
                                            last = target_types_.end();
         i != last;
         ++i, ++p) {
    ...
    result->sources_.push_back(
      build_node::source_t(source_target, sources.front()));
