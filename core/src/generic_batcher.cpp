...
void
generic_batcher::process(const build_node::nodes_t& nodes,
...
typedef boost::unordered_map<pair<const feature_set*, const build_action*>,
                             build_node::sources_t>
  selected_nodes_t;
static void
process_interval(build_node& node,
                 ...
                 unsigned concurency_level)
{
  ...
  for (size_t batched_sources_count = 0;
    ...
    boost::intrusive_ptr<build_node> new_node(
      new build_node(front_node.products_owner(), false));
    ...
    for (; s_iter != s_block_last; ++s_iter, ++batched_sources_count) {
      new_node->sources_.insert(new_node->sources_.end(),
                                s_iter->source_node_->sources_.begin(),
                                s_iter->source_node_->sources_.end());
      new_node->products_.insert(new_node->products_.end(),
                                 s_iter->source_node_->products_.begin(),
                                 s_iter->source_node_->products_.end());
      new_node->dependencies_.insert(
        new_node->dependencies_.end(),
        s_iter->source_node_->dependencies_.begin(),
        s_iter->source_node_->dependencies_.end());
      new_node->down_.insert(new_node->down_.end(),
                             s_iter->source_node_->down_.begin(),
                             s_iter->source_node_->down_.end());
    }
...
void
generic_batcher::process_impl(build_node& node) const
{
  ...
  if (!selected_sources.empty()) {
    ...
    node.down_.erase(unique(node.down_.begin(), node.down_.end()),
                     node.down_.end());
  }

  for (build_node::nodes_t::const_iterator i = node.down_.begin(),
                                           last = node.down_.end();
       i != last;
       ++i) {
