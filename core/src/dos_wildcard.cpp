...
dos_wildcard&
dos_wildcard::assign(const std::string& wild, bool ignore_case)
{
...
// match:
bool
dos_wildcard::match(const char* text) const
{
  return do_match(text, text + std::strlen(text));
}

bool
dos_wildcard::match(const std::string& text) const
{
  return do_match(text.data(), text.data() + text.size());
}

bool
dos_wildcard::match(const filesystem::path& path) const
{
...
// predicates:
bool
dos_wildcard::operator()(const char* text)
{
  return match(text);
}

bool
dos_wildcard::operator()(const std::string& text)
{
  return match(text);
}

bool
dos_wildcard::operator()(const filesystem::path& text)
{
  return match(text);
}
...
bool
dos_wildcard::do_match(const char* p1, const char* p2) const
{
