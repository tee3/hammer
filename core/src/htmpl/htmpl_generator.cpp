                           ...
                           const build_environment& environment) const
{
  const fs::path source_file = [&] {
    source_argument_writer saw(
      {}, source_target_type_, true, source_argument_writer::FULL_PATH, "");
    stringstream s;
  ...
  const fs::path product_file = [&] {
    product_argument_writer paw(
      {},
      *node.targeting_type_,
  ...
  unique_ptr<std::ostream> product_stream = environment.create_output_file(
    product_file.string().c_str(), ios_base::trunc);
  ...
  for (regex_iterator<vector<char>::const_iterator>
         first(content.cbegin(), content.cend(), htmpl_pattern),
       last = {};
       first != last;
       ++first) {
    ...
    if (i == props.end()) {
      environment.error_stream()
        << "[htmpl] Can't find feature '" << feature_name
        << "' for instantiated meta target located at '"
        << node.products_owner().get_meta_target()->location().string()
        << "'.\n"
