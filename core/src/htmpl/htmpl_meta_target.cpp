...
resolve_target_type(const fs::path& filename, engine& e)
{
                        ...
                        const basic_meta_target* this_)
{
  ...
  dependency->set_dependency_data(
    source_decl(target_name, string(), nullptr, nullptr), this_);
...
void
htmpl_meta_target::compute_usage_requirements(
  feature_set& result,
  ...
  const main_target* owner) const
{
  if (type().equal_or_derived_from(types::H)) {
    // generated include dir should apear first, so generated includes will be
    // found earlier
    const fs::path relative_intermediate_dir_path =
      relative_path(constructed_target.intermediate_dir(), location());
    feature* include_intermediate_dir =
      get_engine()->feature_registry().create_feature(
        "include", relative_intermediate_dir_path.string());
