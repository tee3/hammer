...
std::size_t
actuality_checker::check(nodes_t& nodes)
{
  std::size_t result = 0;
...
scanner_context&
actuality_checker::get_scanner_context(const target_type& t, const scanner& s)
{
...
// dependency nodes
void
actuality_checker::mark_to_update(build_node& node,
...
bool
actuality_checker::check(boost::posix_time::ptime& max_target_time,
                         std::size_t& nodes_to_update,
                         build_node& node)
{
  ...
  for (build_node::sources_t::const_iterator i = node.sources_.begin(),
       ...
       i != last;
       ++i) {
    // we skip direct sources that is dependency too. Good example is pch header
    // that can be found in dependency and direct sources
    // we need this because when pch header has been rebuilt sources must be
    // rebuild too.
    if (node.dependencies_.empty() ||
        find(node.dependencies_.begin(),
             node.dependencies_.end(),
             i->source_node_) == node.dependencies_.end()) {
      some_need_to_be_updated =
        check(sources_max_time, nodes_to_update, *i->source_node_) ||
        some_need_to_be_updated;
    }
  }
