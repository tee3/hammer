...
main_target::main_target(const basic_meta_target* mt,
  ...
  : basic_target(this, name, t, props)
  , meta_target_(mt)
...
void
main_target::dependencies(const dependencies_t& deps)
{
  dependencies_ = deps;
}
...
void
main_target::add_additional_dependencies(
  hammer::build_node& generated_node) const
{
...
void
main_target::add_hamfile_dependency(
  hammer::build_node& node,
  const build_node_ptr& intermediate_dir_node) const
{
  boost::intrusive_ptr<hammer::build_node> signature_node(
    new hammer::build_node(*this, false));
...
bool
main_target::add_this_target_dependency(hammer::build_node& node,
                                        const build_nodes_t& nodes) const
{
  for (hammer::build_node::sources_t::iterator i = node.sources_.begin(),
                                               last = node.sources_.end();
       i != last;
       ++i) {
    if (&i->source_node_->products_owner() == this) {
      if (i->source_node_->sources_.empty())
        node.dependencies_.insert(
          node.dependencies_.begin(), nodes.begin(), nodes.end());
      else
...
location_t
main_target::intermediate_dir_impl() const
{
...
main_target::additional_hash_string_data(std::ostream& /*s*/) const
{
  ...
  for (main_target_sources_t::const_iterator i = main_target_sources.begin(),
                                             last = main_target_sources.end();
       i != last;
       ++i)
  ...
  bool first = true;
  for (vector<string>::const_iterator i = hashes.begin(), last = hashes.end();
       i != last;
       ++i) {
    if (!first)
      s << ' ';
