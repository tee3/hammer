  ...
  impl_t::dependencies_t::const_iterator /*last*/) const
{
  ...
  for (; first != last; ++first) {
    ...
    if (i == projects_.end() ||
        (i != projects_.end() && !i->second->has_variant(*first))) {
      auto_ptr<msvc_project> p_guard(
        new msvc_project(engine_,
                         project_output_dir(*(**first).build_node()),
                         variant_names_.front(),
                         owner_->generate_id()));
      msvc_project* p = p_guard.get();
      p->add_variant((**first).build_node());
      p->generate();
      dependencies.insert(
        dependencies.end(), p->dependencies().begin(), p->dependencies().end());
  ...
  if (!dependencies.empty()) {
    ...
    std::sort(
      dependencies.begin(), dependencies.end(), &less_by_location_and_name);
  }
}
...
void
msvc_solution::impl_t::write_project_section(ostream& os,
                                             const msvc_project& project) const
{
  location_t project_path(
    relative_path(project.full_project_name(), output_location_));
  project_path.normalize();

  os << "Project(\"{8BC9CEB8-8B4A-11D0-8D11-00A0C91BC942}\") = \""
...
location_t
msvc_solution::impl_t::project_output_dir(const build_node& node) const
{
  switch (generation_mode_) {
    ...
    case generation_mode::NON_LOCAL: {
      location_t suffix =
        relative_path(output_location_, source_project_.location());
      location_t result =
        node.products_[0]->get_main_target()->location() / suffix;
      result.normalize();
      return result;
    }
...
msvc_solution::msvc_solution(const project& source_project,
...
msvc_solution::add_target(boost::intrusive_ptr<const build_node> /*node*/)
{
  if (!impl_->projects_.empty())
    throw std::runtime_error(
      "MSVC solution generator can handle only one top level target.");
  ...
  p->generate();
  dependencies.insert(
    dependencies.begin(), p->dependencies().begin(), p->dependencies().end());
...
static bool
less_by_name(const msvc_project* lhs, const msvc_project* rhs)
{
  return lhs->name() < rhs->name();
}

void
msvc_solution::write() const
{
  ...
  for (sorted_projects_t::const_iterator i = sorted_projects.begin(),
                                         last = sorted_projects.end();
       i != last;
       ++i) {
    (**i).write();
  ...
  for (impl_t::variant_names_t::const_iterator
         i = impl_->variant_names_.begin(),
         last = impl_->variant_names_.end();
       i != last;
       ++i)
    f << "\t\t" << *i << "|Win32"
      << " = " << *i << "|Win32\n";
...
boost::guid
msvc_solution::generate_id() const
{
