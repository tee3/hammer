...
public:
  ...
  virtual std::unique_ptr<std::ostream> create_output_file(
    const char* filename,
...
static const string post_build_step_format_string(
...
msvc_project::msvc_project(engine& e,
  ...
  , testing_run_passed_type_(
      engine_->get_type_registry().get(types::TESTING_RUN_PASSED))
  ...
  , post_build_step_(post_build_step_format_string)
{
  ...
  boost::shared_ptr<fs_argument_writer> charset(
    new fs_argument_writer("charset", e.feature_registry()));
  ...
  // compiller options
  boost::shared_ptr<fs_argument_writer> optimization(
    new fs_argument_writer("optimization", e.feature_registry()));
  ...
  boost::shared_ptr<fs_argument_writer> inlining(
    new fs_argument_writer("inlining", e.feature_registry()));
  inlining->add("<inlining>off", "0")
    .add("<inlining>on", "1")
    .add("<inlining>full", "2");
  compiller_options_ += inlining;

  boost::shared_ptr<free_feature_arg_writer> includes(
    new free_feature_arg_writer("includes",
                                e.feature_registry().get_def("include"),
                                string(),
                                string(),
                                ";"));
  compiller_options_ += includes;

  boost::shared_ptr<fs_argument_writer> debug_format(
    new fs_argument_writer("debug_format", e.feature_registry()));
  ...
  boost::shared_ptr<fs_argument_writer> warning_level(
    new fs_argument_writer("warning_level", e.feature_registry()));
  warning_level->add("<warnings>on", "3")
    .add("<warnings>off", "0")
    .add("<warnings>all", "4");
  compiller_options_ += warning_level;

  boost::shared_ptr<free_feature_arg_writer> defines(
    new free_feature_arg_writer("defines",
                                e.feature_registry().get_def("define"),
                                string(),
                                string(),
                                ";"));
  compiller_options_ += defines;

  boost::shared_ptr<fs_argument_writer> exception_handling(
    new fs_argument_writer("exception_handling", e.feature_registry()));
  ...
  boost::shared_ptr<fs_argument_writer> runtime(
    new fs_argument_writer("runtime", e.feature_registry()));
  ...
  boost::shared_ptr<fs_argument_writer> rtti(
    new fs_argument_writer("rtti", e.feature_registry()));
  rtti->add("<rtti>on", "true").add("<rtti>off", "false");
  compiller_options_ += rtti;

  boost::shared_ptr<free_feature_arg_writer> cxxflags(
    new free_feature_arg_writer("cxxflags",
                                e.feature_registry().get_def("cxxflags")));
  compiller_options_ += cxxflags;

  boost::shared_ptr<free_feature_arg_writer> cflags(new free_feature_arg_writer(
    "cflags", e.feature_registry().get_def("cflags")));
  compiller_options_ += cflags;

  boost::shared_ptr<pch_argument_writer> pch_header(
    new pch_argument_writer("pch_header"));
  compiller_options_ += pch_header;

  boost::shared_ptr<fs_argument_writer> pch_type(
    new fs_argument_writer("pch_type", e.feature_registry()));
  pch_type->add("<pch>use", "2").add("<pch>create", "1").add("<pch>off", "0");
  compiller_options_ += pch_type;

  // linker options
  boost::shared_ptr<source_argument_writer> additional_libraries(
    new source_argument_writer(
  ...
  boost::shared_ptr<fs_argument_writer> link_debug_info(
    new fs_argument_writer("debug_info", engine_->feature_registry()));
  link_debug_info->add("<debug-symbols>on", "true")
    .add("<debug-symbols>off", "false");
  linker_options_ += link_debug_info;

  boost::shared_ptr<fs_argument_writer> link_subsystem(
    new fs_argument_writer("subsystem", engine_->feature_registry()));
  link_subsystem->add("<user-interface>console", "1")
    .add("<user-interface>gui", "2");
  linker_options_ += link_subsystem;

  boost::shared_ptr<fs_argument_writer> incremental_linking(
    new fs_argument_writer("incremental_linking", engine_->feature_registry()));
  ...
  boost::shared_ptr<free_feature_arg_writer> path_args(
    new free_feature_arg_writer(
      "path_args", e.feature_registry().get_def("testing.input-file")));
  post_build_step_ += path_args;

  boost::shared_ptr<free_feature_arg_writer> non_path_args(
    new free_feature_arg_writer(
      "non_path_args", e.feature_registry().get_def("testing.argument")));
  post_build_step_ += non_path_args;
}

static std::string
make_variant_name(const main_target& mt)
{
...
msvc_project::add_variant(const boost::intrusive_ptr<const build_node>& node)
{
  ...
  if (id_.empty()) {
    ...
    meta_target_relative_to_output_ =
      relative_path(meta_target_->location(), project_output_dir());
...
msvc_project::fill_filters()
{
  ...
  files_.push_back(filter_t(
    source_types, "Source Files", "{4FC737F1-C7A5-4376-A066-2A32D752A2FF}"));
  filter_t::types_t header_types;
  header_types.push_back(&engine_->get_type_registry().get(types::H));
  files_.push_back(filter_t(
    header_types, "Header Files", "{93995380-89BD-4b04-88EB-625FBE52EBFB}"));
...
void
msvc_project::write_header(ostream& s) const
{
...
static void
write_bottom(std::ostream& s)
{
  s << "</VisualStudioProject>";
}

configuration_types::value
msvc_project::resolve_configuration_type(const variant& v) const
{
  const target_type& exe_type = engine_->get_type_registry().get(types::EXE);
  const target_type& static_lib_type =
    engine_->get_type_registry().get(types::STATIC_LIB);
  const target_type& shared_lib_type =
    engine_->get_type_registry().get(types::SHARED_LIB);
  const target_type& header_lib_type =
    engine_->get_type_registry().get(types::HEADER_LIB);
...
static void
write_compiler_options(std::ostream& s,
...
void
msvc_project::write_configurations(std::ostream& s) const
{
  s << "   <Configurations>\n";

  for (variants_t::iterator i = variants_.begin(), last = variants_.end();
       i != last;
       ++i) {
    ...
    if (i->real_node_->targeting_type_->equal_or_derived_from(
          testing_run_passed_type_)) {
      s << "         <Tool\n"
...
static feature_set* compute_file_conf_properties(const basic_target& target,
const msvc_project::variant& v)
{
feature_set* result = v.target_->get_engine()->feature_registry().make_set();
for(feature_set::const_iterator i = target.properties().begin(), last =
target.properties().end(); i != last; ++i)
{
...
*/
void
msvc_project::file_configuration::write(write_context& ctx,
                                        const variant& v) const
{
  if (!v.properties_->contains(target_->properties())) {
    ctx.output_ << "              <FileConfiguration\n"
                << "                   Name=\"" << v.name_ << "\">\n";
    write_compiler_options(
      ctx.output_, ctx.compiller_options_, *node_, ctx.environment_);
    ctx.output_ << "              </FileConfiguration>\n";
  }
}

void
msvc_project::file_with_cfgs_t::write(write_context& ctx,
                                      const std::string& path_prefix) const
{
  ...
  for (file_config_t::const_iterator i = file_config.begin(),
                                     last = file_config.end();
       i != last;
       ++i)
    if (*i->first->properties_ != i->second.target_->properties() &&
        &i->second.target_->type() != &ctx.h_type_) // discard any differences
    // for H targets. Done for
    // PCH. May be FIXME:
    {
                              ...
                              const std::string& /*path_prefix*/) const
{
  ...
  // project file
  typedef std::map<const basic_target*,
                   boost::reference_wrapper<const file_with_cfgs_t>,
                   less_target>
    stabilized_t;
  stabilized_t stabilized;

  for (const files_t::value_type& f : files_)
    stabilized.insert(make_pair(
      f.first, boost::reference_wrapper<const file_with_cfgs_t>(f.second)));

  for (stabilized_t::const_iterator i = stabilized.begin(),
                                    last = stabilized.end();
       i != last;
       ++i)
...
void
msvc_project::write_files(write_context& ctx) const
{
  ctx.output_ << "      <Files>\n";
  string path_prefix = meta_target_relative_to_output_.string();
  for (files_t::const_iterator i = files_.begin(), last = files_.end();
       i != last;
       ++i)
...
msvc_project::write() const
{
  ...
  write_context ctx(f,
                    engine_->get_type_registry().get(types::H),
                    environment,
                    compiller_options_);
  write_files(ctx);
...
void
msvc_project::filter_t::insert(const boost::intrusive_ptr<build_node>& node,
...
msvc_project::gether_files_impl(const build_node& node, variant& v)
{
  ...
  for (iter mi = node.sources_.begin(), mlast = node.sources_.end();
    ...
    if (mi->source_target_->get_meta_target() == meta_target_ ||
        mi->source_target_->get_main_target()->type().equal_or_derived_from(
          obj_type_) ||
        mi->source_target_->get_main_target()->type().equal_or_derived_from(
          pch_type_)) {
      ...
      if (mi->source_target_->get_main_target()->type().equal_or_derived_from(
            searched_lib_)) { // this target is searched lib product
        const std::string& file_name = mi->source_target_->name();
        location_t searched_file(
          mi->source_target_->location().empty()
            ? file_name
            : relative_path(mi->source_target_->location(),
                            project_output_dir_) /
                file_name);
