...
public:
  qt_uic_meta_target(hammer::project* p,
    ...
    : typed_meta_target(p,
                        ...
                        req,
                        usage_req,
                        p->get_engine()->get_type_registry().get(qt_uic_main))
  {
    add_sources(sources);
    set_explicit(true);
  }
...
{
  return new qt_uic_main_target(this, *owner, name(), &type(), properties);
}

void
qt_uic_meta_target::compute_usage_requirements(
  feature_set& result,
  const main_target& constructed_target,
  const feature_set& build_request,
  const feature_set& computed_usage_requirements,
...
{
  typed_meta_target::compute_usage_requirements(result,
                                                constructed_target,
                                                build_request,
                                                computed_usage_requirements,
                                                owner);

  feature* uic_inc = result.owner().create_feature(
    "include", relative_path(owner->intermediate_dir(), location()).string());
  ...
  dependency->set_dependency_data(
    source_decl(name(), std::string(), nullptr, nullptr), this);
...
static sources_decl
convert_H_to_MOCABLE(const sources_decl& src, hammer::project& p)
{
  sources_decl result;
...
public:
  qt_moc_meta_target(hammer::project* p,
    ...
    : alias_meta_target(p,
                        name,
                        convert_H_to_MOCABLE(sources, *p),
                        req,
                        usage_req)
  {
  }
...
static string
determinate_version(const location_t& toolset_home)
{
  for (fs::directory_iterator i = fs::directory_iterator(toolset_home),
        ...
        const string& additional_include_path)
{
  ...
  feature* qt_no_debug_feature =
    e.feature_registry().create_feature("define", "QT_NO_DEBUG");
#if defined(_WIN32)
  feature* top_include_feature =
    e.feature_registry().create_feature("include", "./include");
  feature* include_feature = e.feature_registry().create_feature(
    "include", "./include/" + additional_include_path);
#else
  feature* top_include_feature =
    e.feature_registry().create_feature("include", "./include/" + include_tag);
  feature* include_feature = e.feature_registry().create_feature(
  ...
  auto_ptr<prebuilt_lib_meta_target> lib_debug(
    new prebuilt_lib_meta_target(&qt_project,
                                 lib_name,
                                 "./lib/" + lib_name + "d" + lib_tag + ".lib",
                                 debug_req,
                                 requirements_decl()));
  auto_ptr<prebuilt_lib_meta_target> lib_release(
    new prebuilt_lib_meta_target(&qt_project,
                                 lib_name,
                                 "./lib/" + lib_name + lib_tag + ".lib",
                                 release_req,
                                 requirements_decl()));
...
static void
add_types_and_generators(engine& e,
...
{
  // register qt types
  // FIXME: we don't derive qt mocable from H because current generator
  // implementation can't properly choose qt.moc generator
  ...
  e.get_type_registry().insert(target_type(
    qt_rced_cpp, ".cpp", e.get_type_registry().get(types::CPP), "qrc_"));
  e.get_type_registry().insert(
    target_type(qt_uiced_h, ".h", e.get_type_registry().get(types::H), "ui_"));
  e.get_type_registry().insert(target_type(qt_uic_main, ""));

  // QT_UI -> QT_UICED_H
  {
    shared_ptr<source_argument_writer> ui_source(new source_argument_writer(
      "ui_source", e.get_type_registry().get(qt_ui)));
    shared_ptr<product_argument_writer> uic_product(new product_argument_writer(
    ...
    unique_ptr<cmdline_action> uic_action(
      new cmdline_action("qt.uic", uic_product));
    *uic_action += uic_cmd;
  ...
  // QT_RC -> QT_RCED_CPP
  {
    shared_ptr<source_argument_writer> rcc_source(new source_argument_writer(
      "rcc_source", e.get_type_registry().get(qt_rc)));
    shared_ptr<product_argument_writer> rcc_product(new product_argument_writer(
    ...
    unique_ptr<cmdline_action> rcc_action(
      new cmdline_action("qt.rcc", rcc_product));
    *rcc_action += rcc_cmd;
  ...
  // QT_MOCABLE -> CPP
  {
    shared_ptr<source_argument_writer> mocable_source(
      new source_argument_writer("mocable_source",
                                 e.get_type_registry().get(qt_mocable)));
    shared_ptr<product_argument_writer> cpp_product(new product_argument_writer(
    ...
    unique_ptr<cmdline_action> moc_action(
      new cmdline_action("qt.moc", cpp_product));
    *moc_action += moc_cmd;
  ...
  e.call_resolver().insert(
    "qt.moc",
    boost::function<void(project*,
                         const string&,
                         const sources_decl&,
                         requirements_decl*,
                         requirements_decl*)>(&qt_moc_rule));
  e.call_resolver().insert(
    "qt.uic",
    boost::function<sources_decl(project*, const sources_decl&)>(&qt_uic_rule));
}

void
qt_toolset::init_impl(engine& e,
