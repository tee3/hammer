...
void
gcc_toolset::init_impl(engine& e,
                       ...
                       const location_t* toolset_home) const
{
  ...
  // C -> OBJ
  {
    shared_ptr<source_argument_writer> c_input(
      new source_argument_writer("c_input",
    ...
    *obj_action += obj_cmd;
    ...
    std::unique_ptr<generator> g_copy(
      new generator(e,
                    generator_prefix + ".compiler.c",
                    source,
                    target,
                    false,
                    generator_condition));
    std::unique_ptr<cmdline_action> obj_action_copy(
      new cmdline_action(*obj_action));

    g->action(std::move(obj_action));

    add_compile_fail_generator(
      e, std::move(g_copy), std::move(obj_action_copy));
    e.generators().insert(std::move(g));
  }

  // CPP -> OBJ
  {
    shared_ptr<fs_argument_writer> cxxflags(
      new fs_argument_writer("cxxflags", e.feature_registry()));
  ...
  // ... -> SHARED_LIB
  {
    ...
    shared_ptr<product_argument_writer> shared_lib_product(
      new product_argument_writer(
        "shared_lib_product", e.get_type_registry().get(types::SHARED_LIB)));
    shared_ptr<unix_libraries_argument_writer> libraries_writer(
      new unix_libraries_argument_writer("libraries", linker_type::GNU, e));
    cmdline_builder shared_lib_cmd(
      install_data.linker_.string() +
      " -shared $(link_flags) $(searched_lib_searched_dirs) -o "
      "\"$(shared_lib_product)\" $(obj_sources) $(libraries)\n");
    ...
    unique_ptr<cmdline_action> shared_lib_action(
      new cmdline_action("link-shared-lib", shared_lib_product));
  ...
  // ... -> EXE
  {
    ...
    unique_ptr<cmdline_action> exe_action(
      new cmdline_action("link-exe", exe_product));
    cmdline_builder exe_cmd(
      "LD_LIBRARY_PATH=$(ld_library_path_dirs):LD_LIBRARY_PATH " +
      install_data.linker_.string() +
      " $(link_flags) $(searched_lib_searched_dirs) -o \"$(exe_product)\" "
      "$(obj_sources) $(libraries)\n");
  ...
  // ... -> STATIC_LIB
  {
    ...
    cmdline_builder static_lib_cmd(
      install_data.librarian_.string() +
      " $(user_archive_flags) rc $(static_lib_product) $(obj_sources)");

    static_lib_cmd += static_lib_product;
    static_lib_cmd += obj_sources;
    static_lib_cmd += user_archive_flags;

    unique_ptr<cmdline_action> static_lib_action(
      new cmdline_action("link-static-lib", static_lib_product));
...
void
gcc_toolset::autoconfigure(engine& e) const
{
