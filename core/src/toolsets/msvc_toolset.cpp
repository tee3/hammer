...
void
msvc_toolset::init_impl(engine& e,
                   ...
                   const location_t& toolset_home) const
{
  ...
  shared_ptr<source_argument_writer> searched_lib_sources(
    new source_argument_writer("searched_lib_sources",
                               e.get_type_registry().get(types::SEARCHED_LIB),
                               false,
                               source_argument_writer::WITHOUT_PATH));
  shared_ptr<source_argument_writer> prebuilt_lib_sources(
    new source_argument_writer(
      "prebuilt_lib_sources",
      e.get_type_registry().get(types::PREBUILT_STATIC_LIB),
      true,
      source_argument_writer::FULL_PATH));
  shared_ptr<source_argument_writer> import_lib_sources(
    new source_argument_writer("import_lib_sources",
                               e.get_type_registry().get(types::IMPORT_LIB),
                               true,
                               source_argument_writer::FULL_PATH));

  shared_ptr<product_argument_writer> obj_product(new product_argument_writer(
    "obj_product", e.get_type_registry().get(types::OBJ)));
  shared_ptr<product_argument_writer> pch_product(new product_argument_writer(
    "pch_product", e.get_type_registry().get(types::PCH)));
  ...
  shared_ptr<fs_argument_writer> cppflags(
    new fs_argument_writer("cppflags", e.feature_registry()));
  cppflags
    ->add("<exception-handling>on/<asynch-exceptions>off/<extern-c-nothrow>off",
          "/EHs")
    .add("<exception-handling>on/<asynch-exceptions>off/<extern-c-nothrow>on",
         "/EHsc")
    .add("<exception-handling>on/<asynch-exceptions>on/<extern-c-nothrow>off",
         "/EHa")
    .add("<exception-handling>on/<asynch-exceptions>on/<extern-c-nothrow>on",
         "/EHac");

  shared_ptr<free_feature_arg_writer> user_c_flags(new free_feature_arg_writer(
    "user_c_flags", e.feature_registry().get_def("cflags")));
  shared_ptr<free_feature_arg_writer> user_link_flags(
    new free_feature_arg_writer("user_link_flags",
                                e.feature_registry().get_def("linkflags")));
  shared_ptr<free_feature_arg_writer> user_cxx_flags(
    new free_feature_arg_writer("user_cxx_flags",
                                e.feature_registry().get_def("cxxflags")));
  shared_ptr<free_feature_arg_writer> user_archive_flags(
    new free_feature_arg_writer("user_archive_flags",
  ...
  // CPP -> OBJ
  {
    ...
    unique_ptr<cmdline_action> obj_action(
      new cmdline_action("compile-c++", obj_product));
    *obj_action += setup_vars;
    *obj_action += obj_cmd;

    boost::shared_ptr<batched_cmdline_action> batched_obj_action(
      new batched_cmdline_action("batched compile-c++"));
  ...
  // CPP + H -> PCH + OBJ
  {
    ...
    obj_cmd += output_dir;
    unique_ptr<cmdline_action> obj_action(
      new cmdline_action("compile-c++-pch", obj_product));
    *obj_action += setup_vars;
    *obj_action += obj_cmd;
  ...
  // RC -> RES
  {
    shared_ptr<source_argument_writer> rc_source(new source_argument_writer(
    ...
    unique_ptr<cmdline_action> res_action(
      new cmdline_action("compile-rc", res_product));
  ...
  // ... -> EXE
  {
    shared_ptr<source_argument_writer> obj_sources(new source_argument_writer(
      "obj_sources", e.get_type_registry().get(types::OBJ)));
    shared_ptr<product_argument_writer> exe_product_unc(
      new product_argument_writer(
        "exe_product_unc",
        e.get_type_registry().get(types::EXE),
        product_argument_writer::output_strategy::FULL_PATH));
    shared_ptr<product_argument_writer> exe_product(new product_argument_writer(
      "exe_product", e.get_type_registry().get(types::EXE)));
    shared_ptr<product_argument_writer> exe_manifest_product(
      new product_argument_writer(
    ...
    unique_ptr<cmdline_action> exe_action(
      new cmdline_action("link-exe", exe_product, exe_rsp));
  ...
  // ... -> STATIC_LIB
  {
    shared_ptr<source_argument_writer> obj_sources(new source_argument_writer(
      "obj_sources", e.get_type_registry().get(types::OBJ)));
    shared_ptr<product_argument_writer> static_lib_product(
      new product_argument_writer(
        "static_lib_product", e.get_type_registry().get(types::STATIC_LIB)));
    shared_ptr<product_argument_writer> static_lib_product_unc(
      new product_argument_writer(
        "static_lib_product_unc",
        e.get_type_registry().get(types::STATIC_LIB),
        product_argument_writer::output_strategy::FULL_PATH));
    cmdline_builder static_lib_rsp(
      "$(user_archive_flags) /out:\"$(static_lib_product)\"\n$(obj_sources)");
    ...
    unique_ptr<cmdline_action> static_lib_action(new cmdline_action(
      "link-static-lib", static_lib_product, static_lib_rsp));
  ...
  // ... -> SHARED_LIB IMPORT_LIB
  {
    shared_ptr<source_argument_writer> obj_sources(new source_argument_writer(
      "obj_sources", e.get_type_registry().get(types::OBJ)));
    shared_ptr<product_argument_writer> import_lib_product(
      new product_argument_writer(
        "import_lib_product", e.get_type_registry().get(types::IMPORT_LIB)));
    shared_ptr<product_argument_writer> shared_lib_rel_product(
      new product_argument_writer(
        "shared_lib_rel_product",
        e.get_type_registry().get(types::SHARED_LIB)));
    shared_ptr<product_argument_writer> shared_lib_product(
      new product_argument_writer(
        "shared_lib_product",
        e.get_type_registry().get(types::SHARED_LIB),
        product_argument_writer::output_strategy::FULL_PATH));
    shared_ptr<product_argument_writer> dll_manifest_product(
      new product_argument_writer(
        "dll_manifest_product",
        e.get_type_registry().get(types::DLL_MANIFEST)));
    cmdline_builder shared_lib_cmd(
      config_data.linker_.string() + " \"@$(shared_lib_rel_product).rsp\"\n"
                                     "if %ERRORLEVEL% NEQ 0 EXIT %ERRORLEVEL%\n"
    ...
    unique_ptr<cmdline_action> shared_lib_action(new cmdline_action(
      "link-shared-lib", shared_lib_rel_product, shared_lib_rsp));
    ...
    *shared_lib_action += shared_lib_cmd;
    ...
    unique_ptr<generator> g(
      new exe_and_shared_lib_generator(e,
                                       generator_prefix + ".linker.shared_lib",
                                       source,
                                       target,
                                       true,
                                       generator_condition));
...
void
msvc_toolset::autoconfigure(engine& e) const
{
  using namespace std;
