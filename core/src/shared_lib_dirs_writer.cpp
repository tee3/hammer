                                   ...
                                   const build_environment& environment) const
{
  ...
  std::vector<const hammer::target_type*> types_to_collect(1,
                                                           &shared_lib_type_);
  // FIXME: const problems...
  std::vector<boost::intrusive_ptr<build_node>> sources(
    1, boost::intrusive_ptr<build_node>(const_cast<build_node*>(&node)));

  collect_nodes(
    collected_nodes, visited_nodes, sources, types_to_collect, true);

  bool is_first = true;
  for (build_node::sources_t::const_iterator i = collected_nodes.begin(),
