...
static void
fillBuffer(pANTLR3_COMMON_TOKEN_STREAM tokenStream, int k, bool with_skiping);

static void
nbs_free(struct ANTLR3_COMMON_TOKEN_STREAM_struct* tokenStream)
{
...
* we store the value + 1 cast to a void *. Hence on systems where NULL =
* (void *)0
...
static pANTLR3_COMMON_TOKEN
nbs_tokLT(pANTLR3_TOKEN_STREAM ts, ANTLR3_INT32 k)
{
  ANTLR3_INT32 i;
  ANTLR3_INT32 n;
...
void
non_buffered_token_stream::relex_from_current()
{
  pANTLR3_COMMON_TOKEN tok =
    ctx_.tstream_->tstream->_LT(ctx_.tstream_->tstream, -1);
