...
bool
parser::parse_raw_script(const std::string& script_body,
                         const std::string& script_name)
{
  reset();

  impl_->input_ =
    antlr3NewAsciiStringInPlaceStream((pANTLR3_UINT8)script_body.c_str(),
                                      script_body.size(),
...
parser::parse(const char* file_name)
{
  ...
  ////      tstream_ = antlr3CommonTokenStreamSourceNew(ANTLR3_SIZE_HINT,
  /// TOKENSOURCE(lexer_));
  //      impl_->tstream_ = non_buffered_token_stream::create(ANTLR3_SIZE_HINT,
  //      TOKENSOURCE(impl_->lexer_));
  //      impl_->parser_ = hammerParserNew(impl_->tstream_);
  //      details::hammer_parser_context ctx;
  //      ctx.base_displayRecognitionError =
  //      impl_->parser_->pParser->rec->displayRecognitionError;
  //      ctx.token_stream_ =
  //      static_cast<non_buffered_token_stream*>(impl_->tstream_->super);
  //      ctx.token_stream_->ctx_.input_ = impl_->input_;
  //      ctx.token_stream_->ctx_.lexer_ = impl_->lexer_->pLexer;
  //      impl_->parser_->pParser->rec->displayRecognitionError =
  //      &displayRecognitionError;
  //      impl_->lexer_->pLexer->super =
  //      &static_cast<non_buffered_token_stream*>(impl_->tstream_->super)->ctx_;
...
parser::impl_t::parse()
{
  ...
  //      TOKENSOURCE(lexer_));
  tstream_ =
    non_buffered_token_stream::create(ANTLR3_SIZE_HINT, TOKENSOURCE(lexer_));
  parser_ = hammerParserNew(tstream_);
  details::hammer_parser_context ctx;
  ctx.base_displayRecognitionError =
    parser_->pParser->rec->displayRecognitionError;
  ...
  lexer_->pLexer->super =
    &static_cast<non_buffered_token_stream*>(tstream_->super)->ctx_;
  ctx.engine_ = engine_;
