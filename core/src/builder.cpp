...
void
builder::impl_t::task_handler(shared_ptr<worker_ctx_t> ctx)
{
  if (ctx->node().action()) {
...
mark_deps_failed_to_build(build_queue_node_t& node)
{
  ...
  for (build_queue_nodes_t::iterator i = node.uses_nodes_.begin(),
                                     last = node.uses_nodes_.end();
       i != last;
       ++i)
...
void
builder::impl_t::task_completition_handler(shared_ptr<worker_ctx_t> ctx)
{
  if (interrupt_flag_)
    return;
                           ...
                           const project* bounds)
{
  ...
  for (const build_queue_node_t* n : all_build_nodes) {
    ...
    labels +=
      "|{action |{ " +
      (n->node_->action() ? n->node_->action()->name() : string("null")) + "}}";
