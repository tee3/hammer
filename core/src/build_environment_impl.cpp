  ...
  try {
#if defined(_WIN32)
    {
      std::unique_ptr<ostream> f(create_output_file(
        full_tmp_file_name.string().c_str(), std::ios_base::out));

      for (vector<string>::const_iterator i = cmds.begin(), last = cmds.end();
           i != last;
           ++i)
        *f << *i << '\n';
    }
    ...
    if (captured_output_stream != NULL) {
      ...
      tg.create_thread(boost::bind(&stream_copy_thread,
                                   boost::ref(shell_action_child.get_stdout()),
                                   boost::ref(*captured_output_stream)));
      if (captured_error_stream != NULL)
        tg.create_thread(
          boost::bind(&stream_copy_thread,
                      boost::ref(shell_action_child.get_stderr()),
                      boost::ref(*captured_error_stream)));
    ...
    if (st.exit_status() != 0)
#if defined(_WIN32)
      dump_shell_command(captured_error_stream ? *captured_error_stream : cerr,
                         full_tmp_file_name);
#else
...
open_input_stream(const location_t& full_content_file_name)
{
  ...
  wstring unc_path(
    L"\\\\?\\" + to_wide(location_t(full_content_file_name)).string<wstring>());
...
void
build_environment_impl::dump_shell_command(
  std::ostream& s,
...
void
build_environment_impl::create_directories(
  const location_t& dir_to_create) const
{
  boost::filesystem::create_directories(dir_to_create);
}

void
build_environment_impl::remove(const location_t& p) const
{
  boost::filesystem::remove(p);
}

void
build_environment_impl::remove_file_by_pattern(const location_t& dir,
                                               const std::string& pattern) const
{
  ...
  for (iter i = iter(dir), last = iter(); i != last; ++i) {
    ...
    if (!is_directory(st) &&
        boost::regex_match(i->path().filename().string(), rpattern))
      remove(i->path());
  }
}

void
build_environment_impl::copy(const location_t& full_source_path,
...
bool
build_environment_impl::write_tag_file(const std::string& filename,
...
location_t
build_environment_impl::working_directory(const basic_target& t) const
{
