...
testing_compile_fail_meta_target::testing_compile_fail_meta_target(
  project* p,
  ...
  : typed_meta_target(
      ...
      req,
      usage_req,
      p->get_engine()->get_type_registry().get(types::TESTING_COMPILE_FAIL))
{
}

void
testing_compile_fail_meta_target::instantiate_impl(
  ...
  feature_set* usage_requirements) const
{
  ...
  for (const feature* f : build_request) {
    if (f->name() == "testing.additional-source") {
      feature* source =
        get_engine()->feature_registry().create_feature("use", "");
      source->set_dependency_data(f->get_dependency_data().source_,
