...
void
cmdline_builder::write(std::ostream& output,
                       ...
                       const build_environment& environment) const
{
  ...
  if (!parse(c_str_cmd,
             ...
             *(anychar_p - str_p("$(")) >>
               !list_p(str_p("$(") >>
                         (+(anychar_p - ')'))[push_back_a(arguments)] >> ')',
                       *(anychar_p - "$(")) >>
               *anychar_p)
         .full) {
    throw std::runtime_error(
      "[cmdline_builder] Can't parse command template '" + cmd_ + "'");
  }

  for (arguments_t::const_iterator i = arguments.begin(),
                                   last = arguments.end();
       i != last;
       ++i) {
