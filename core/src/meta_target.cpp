...
void
remove_duplicates(deduplicator_t& deduplicator, meta_targets_t& targets)
{
...
static void
instantiate_meta_targets(sources_decl& simple_targets,
...
{
  feature_set& local_usage_requirements =
    *this_.get_engine()->feature_registry().make_set();
  instantiate_meta_targets(meta_targets,
                           build_request,
                           &owner_for_new_targets,
                           &instantiated_meta_targets,
                           &local_usage_requirements);
  ...
  if (!sources_from_features.empty()) {
    ...
    this_.split_sources(
      &simple_targets, &new_meta_targets, sources_from_features, build_request);
...
void
compute_additional_usage_requirements(
  ...
  const main_target& owner_for_new_targets)
{
  ...
  this_.split_sources(&ignored_simple_targets,
                      &ignored_meta_targets,
                      sources_from_usage,
                      build_request);
  ...
  feature_set* local_usage_requirements =
    this_.get_engine()->feature_registry().make_set();
  instantiate_meta_targets(ignored_meta_targets,
                           build_request,
                           &owner_for_new_targets,
                           &ignored_instantiated_meta_targets,
                           local_usage_requirements);
  ...
  if (!sources_from_usage_requirements.empty()) {
    ...
    this_.split_sources(&simple_targets,
                        &meta_targets,
...
static void
transfer_public_sources(feature_set& dest,
                        const sources_decl& sources,
                        const feature_set& build_request,
                        feature_registry& fr,
                        const basic_meta_target& relative_to_target)
{
  // when transferring public sources we should make <use> with current build
  // request applied
  // because this is the only way to produce correct usage requirements in
  // dependent targets
...
void
meta_target::instantiate_impl(const main_target* owner,
                              const feature_set& build_request_param,
                              std::vector<basic_target*>* result,
                              feature_set* usage_requirements) const
{
  const feature_set& build_request =
    build_request_param.has_undefined_features()
  ...
  feature_set* build_request_for_dependencies =
    get_engine()->feature_registry().make_set();
  ...
  split_sources(
    &simple_targets, &meta_targets, sources(), *build_request_for_dependencies);
  split_sources(&simple_targets,
                &meta_targets,
                sources_from_requirements,
  ...
  split_sources(&simple_targets,
                ...
                dependencies_from_requierements,
                *build_request_for_dependencies);
  ...
  main_target* mt = construct_main_target(
    owner, mt_fs); // construct_main_target may construct main_target with
  ...
  extract_dependencies(
    dependencies_from_instantiations, *local_usage_requirements, *this);
  split_sources(&simple_targets,
                &dependency_meta_targets,
                dependencies_from_instantiations,
                *build_request_for_dependencies);
  ...
  if (!dependency_meta_targets.empty())
    instantiate_meta_targets(simple_targets,
                             ...
                             sources_from_features,
                             *ignored_dependencies_usage_requirements,
                             *this,
                             dependency_meta_targets,
                             *build_request_for_dependencies,
                             *mt);
  ...
  mt->properties(mt_fs);
  instantiate_simple_targets(
    simple_targets, *mt_fs, *mt, &instantiated_meta_targets);
  ...
  transfer_public_sources(*usage_requirements,
                          all_sources,
                          *build_request_for_dependencies,
                          get_engine()->feature_registry(),
                          *this);
  compute_usage_requirements(*usage_requirements,
                             *mt,
                             *build_request_for_dependencies,
                             *local_usage_requirements,
                             owner);

  // we need to transform references in dependency features to local
  // meta-targets to './/foo' form
...
void
meta_target::compute_usage_requirements(
  feature_set& result,
