...
sources_decl::sources_decl(const sources_decl& rhs)
  : impl_(rhs.impl_)
{
  ++impl_->ref_counter_;
}

sources_decl::const_iterator
sources_decl::begin() const
{
  return impl_->values_.begin();
}

sources_decl::const_iterator
sources_decl::end() const
{
  return impl_->values_.end();
}

sources_decl::iterator
sources_decl::begin()
{
  return impl_->values_.begin();
}
...
void
sources_decl::push_back(const std::string& v, const type_registry& tr)
{
  clone_if_needed();
...
void
sources_decl::push_back(const source_decl& v)
{
  clone_if_needed();
...
void
sources_decl::add_to_source_properties(const feature_set& props)
{
  clone_if_needed();
...
void
sources_decl::transfer_from(sources_decl& s)
{
  clone_if_needed();
  impl_->values_.insert(
    impl_->values_.end(), s.impl_->values_.begin(), s.impl_->values_.end());
  s.clear();
}

void
sources_decl::insert(const sources_decl& s)
{
  clone_if_needed();
  impl_->values_.insert(
    impl_->values_.end(), s.impl_->values_.begin(), s.impl_->values_.end());
...
sources_decl::unique()
{
  ...
  impl_->values_.erase(
    std::unique(impl_->values_.begin(), impl_->values_.end()),
    impl_->values_.end());
}

bool
sources_decl::empty() const
{
  return impl_->values_.empty();
}

std::size_t
sources_decl::size() const
{
