...
void
register_standart_types(type_registry& tr, feature_registry& fr)
{
  ...
  target_type::suffixes_t obj_suffixes = list_of<target_type::suffix_def>(
    ".o", *parse_simple_set("<host-os>linux", fr))(
  ...
  target_type::suffixes_t shared_suffixes = list_of<target_type::suffix_def>(
    ".dll", *parse_simple_set("<host-os>windows", fr))(
    ".so", *parse_simple_set("<host-os>linux", fr));
  const target_type& tSHARED_LIB =
    tr.insert(target_type(SHARED_LIB, shared_suffixes, tLIB));
  tr.insert(target_type(PREBUILT_SHARED_LIB, "", tSHARED_LIB));
  const target_type& tSTATIC_LIB =
    tr.insert(target_type(STATIC_LIB, ".lib", tLIB));
  tr.insert(target_type(PREBUILT_STATIC_LIB, "", tSTATIC_LIB));

  target_type::suffixes_t static_lib_suffixes =
    list_of<target_type::suffix_def>(".lib",
                                     *parse_simple_set("<host-os>windows", fr))(
      ".a", *parse_simple_set("<host-os>linux", fr));
  tr.insert(target_type(IMPORT_LIB, static_lib_suffixes, tSTATIC_LIB));
  const target_type& tSEARCHED_LIB =
    tr.insert(target_type(SEARCHED_LIB, "", tLIB));
  ...
  target_type::suffixes_t exe_suffixes = list_of<target_type::suffix_def>(
    ".exe", *parse_simple_set("<host-os>windows", fr))(
