...
void
on_nested_rule_enter(pANTLR3_PARSER parser)
{
  MAKE_CTX();
...
void
on_nested_rule_leave(pANTLR3_PARSER parser)
{
  MAKE_CTX();
...
bool
argument_is_string(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  return ctx.rule_context_.rule_->second->args()
           .at(ctx.rule_context_.arg_)
           .ti() == typeid(string);
}

bool
argument_is_string_list(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  return ctx.rule_context_.rule_->second->args()
           .at(ctx.rule_context_.arg_)
           .ti() == typeid(vector<string>);
}

bool
argument_is_feature(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  return ctx.rule_context_.rule_->second->args()
           .at(ctx.rule_context_.arg_)
           .ti() == typeid(hammer::feature);
}

bool
argument_is_requirements(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  return ctx.rule_context_.rule_->second->args()
           .at(ctx.rule_context_.arg_)
           .ti() == typeid(hammer::requirements_decl);
}

bool
argument_is_project_requirements(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  return ctx.rule_context_.rule_->second->args()
           .at(ctx.rule_context_.arg_)
           .ti() == typeid(hammer::project_requirements_decl);
}

bool
argument_is_sources(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  return ctx.rule_context_.rule_->second->args()
           .at(ctx.rule_context_.arg_)
           .ti() == typeid(hammer::sources_decl);
}

bool
is_conditional_feature(pANTLR3_PARSER parser)
{
...
void
enter_sources_decl(pANTLR3_PARSER parser)
{
  MAKE_CTX();
...
void
leave_sources_decl(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  ctx.token_stream_->ctx_.source_lexing_.pop();
  ctx.token_stream_->relex_from_current();
...
void
enter_rule_invoke(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  ctx.token_stream_->ctx_.source_lexing_.push(false);
  ctx.token_stream_->relex_from_current();
...
void
leave_rule_invoke(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  ctx.token_stream_->ctx_.source_lexing_.pop();
  ctx.token_stream_->relex_from_current();
...
bool
is_path_element(pANTLR3_PARSER parser)
{
...
bool
is_path_slash(pANTLR3_PARSER parser)
{
  pANTLR3_COMMON_TOKEN t_1 = parser->tstream->_LT(parser->tstream, -1);
  pANTLR3_COMMON_TOKEN t1 = parser->tstream->_LT(parser->tstream, 1);
  pANTLR3_COMMON_TOKEN t2 = parser->tstream->_LT(parser->tstream, 2);
...
bool
is_trailing_slash(pANTLR3_PARSER parser)
{
  pANTLR3_COMMON_TOKEN t1 = parser->tstream->_LT(parser->tstream, 1);
  pANTLR3_COMMON_TOKEN t2 = parser->tstream->_LT(parser->tstream, 2);
  return t1->stop + 1 != t2->start;
...
bool
is_head_slash(pANTLR3_PARSER parser)
{
  pANTLR3_COMMON_TOKEN t1 = parser->tstream->_LT(parser->tstream, 1);
  pANTLR3_COMMON_TOKEN t2 = parser->tstream->_LT(parser->tstream, 2);
  return t1->stop + 1 == t2->start;
...
bool
is_dependency_feature(pANTLR3_PARSER parser)
{
  MAKE_CTX();
  pANTLR3_COMMON_TOKEN feature_name_token =
