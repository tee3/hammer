...
rfind(const std::string& where, const std::string& what)
{
  ...
  while (where_first <= where_last && what_first <= what_last) {
...
// FIXME: need totally rewrite this logic in very efficiently way because it
// calls many times
const target_type*
type_registry::resolve_from_target_name(const std::string& name) const
{
  for (types_t::const_iterator i = types_.begin(), last = types_.end();
    ...
    for (target_type::suffixes_t::const_iterator
           j = i->second->suffixes().begin(),
           j_last = i->second->suffixes().end();
         j != j_last;
         ++j) {
      string::size_type p =
        rfind(name, j->suffix_); // s_name.rfind(j->c_str());
      if (p != string::npos && p + j->suffix_.size() == name.size())
        return i->second;
    }
  }
...
const target_type&
type_registry::soft_resolve_from_target_name(const std::string& name) const
{
...
const target_type&
type_registry::hard_resolve_from_target_name(const std::string& name) const
{
  const target_type* t = resolve_from_target_name(name);
  if (t == NULL)
...
const target_type*
type_registry::resolve_from_suffix(const char* first, const char* last) const
{
  return resolve_from_suffix(string(first, last));
}

const target_type*
type_registry::resolve_from_suffix(const std::string& suffix) const
{
...
const target_type*
type_registry::find(const type_tag& tag) const
{
...
const target_type&
type_registry::get(const type_tag& tag) const
{
...
const target_type&
type_registry::insert(const target_type& a_t)
{
  ...
  if (i.second) {
    if (!t->suffixes().empty()) {
      for (target_type::suffixes_t::const_iterator i = t->suffixes().begin(),
                                                   last = t->suffixes().end();
           i != last;
           ++i)
    ...
    throw std::runtime_error("[type_registry] Can't add type '" +
                             t->tag().name() + "' twice");
}
}
