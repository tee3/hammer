...
directory_target::generate() const
{
  throw std::runtime_error(
    "[directory_target::generate] This method must not be invoked");
}

void
directory_target::timestamp_info_impl() const
{
  ...
  if (exists(dir_to_create_)) {
    ...
    timestamp_info_.timestamp_ =
      boost::posix_time::ptime(boost::date_time::neg_infin);
  }
}
} // namespace hammer
