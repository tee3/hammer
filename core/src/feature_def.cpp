...
bool
feature_def::is_legal_value(const std::string& v) const
{
  return std::find(legal_values_.begin(), legal_values_.end(), v) !=
         legal_values_.end();
}

void
feature_def::compose(const std::string& value, feature_set* c)
{
...
{
  return const_cast<subfeature_def&>(
    const_cast<const feature_def*>(this)->get_subfeature(name));
}
