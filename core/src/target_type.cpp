...
target_type::target_type(const type_tag& tag,
...
{
  for (target_type::prefixes_t::const_iterator i = prefixes_.begin(),
                                               last = prefixes_.end();
       i != last;
       ++i) {
    if (i->condition_ == NULL || environment.contains(*i->condition_))
...
target_type::clone(const type_registry& tr) const
{
  ...
  std::auto_ptr<target_type> result(
    base() == NULL ? new target_type(tag(), suffixes())
