...
void
project::location(const location_t& l)
{
...
void
project::add_targets_as_explicit(bool v)
{
...
int
compute_alternative_rank(const feature_set& target_properties,
...
static bool
s_great(const project::selected_target& lhs,
...
void
error_cannot_choose_alternative(const project& p,
                                const std::string& target_name,
                                const feature_set& build_request_param)
{
  auto fmt = boost::format(
    "Failed to selecting best alternatives for target '%s' in project '%s'\n"
    "Build request: %s\n");

  stringstream s;
  dump_for_hash(s, build_request_param);
  throw std::runtime_error((fmt % target_name % p.location() % s.str()).str());
}
...
bool
project::operator==(const project& rhs) const
{
  return this == &rhs;
}
...
void
project::mark_as_explicit(const std::string& name)
{
