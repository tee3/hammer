...
static_lib_generator::static_lib_generator(
  hammer::engine& e,
...
build_nodes_t
static_lib_generator::construct(
  ...
  const main_target& owner) const
{
  ...
  build_nodes_t result(generator::construct(
    type_to_construct, props, modified_sources, t, name, owner));
  result.insert(
    result.end(), extracted_products.begin(), extracted_products.end());
