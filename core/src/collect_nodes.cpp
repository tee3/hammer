              ...
              bool recursive)
{
  ...
  for (nodes_t::const_iterator i = sources.begin(), last = sources.end();
       i != last;
       ++i) {
    if (visited_nodes.find(i->get()) == visited_nodes.end()) {
      local_visited_nodes.insert(i->get());
      for (build_node::sources_t::const_iterator s = (**i).sources_.begin(),
                                                 s_last = (**i).sources_.end();
           s != s_last;
           ++s)
        new_sources.push_back(s->source_node_);

      new_sources.insert(new_sources.end(),
                         (**i).dependencies_.begin(),
                         (**i).dependencies_.end());
    } else
      continue;

    for (build_node::targets_t::const_iterator p = (**i).products_.begin(),
                                               p_last = (**i).products_.end();
         p != p_last;
         ++p) {
      vector<const target_type*>::const_iterator t = std::find(
