...
exe_and_shared_lib_generator::exe_and_shared_lib_generator(
  hammer::engine& e,
...
build_nodes_t
exe_and_shared_lib_generator::construct(
  ...
  const main_target& owner) const
{
  ...
  for (build_sources_t::const_iterator i = modified_sources.begin();
    ...
    if ((**i).targeting_type_->equal_or_derived_from(searched_lib_)) {
      // searched_lib may have <search> feature - add it to current build
      // request
