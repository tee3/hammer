...
void
init_instantiation_tests(const fs::path& test_data_root);
void
...
static void
complete_build_test_function(const fs::path& working_directory)
{
...
init_unit_test_suite(int argc, char* argv[])
{
  ...
  add_tests_from_filesystem(
    test_data_path / "htmpl",
    "htmpl",
    bind(complete_build_test_function, placeholders::_1));
  add_tests_from_filesystem(
    test_data_path / "qt",
    "qt",
    bind(complete_build_test_function, placeholders::_1));
