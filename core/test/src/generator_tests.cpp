...
static void
compare_files(const fs::path& lhs,
              const fs::path& rhs,
              const fs::path& test_root)
{
  ...
  if (file_size(lhs) != file_size(rhs)) {
    BOOST_CHECK_MESSAGE(file_size(lhs) == file_size(rhs),
                        lhs.string() + " != " + rhs.string());
    return;
  }

  bool result = std::equal(istreambuf_iterator<char>(lhs_f),
                           istreambuf_iterator<char>(),
                           istreambuf_iterator<char>(rhs_f));

  if (!result) {
    ostringstream s;
    s << "File '" << relative_path(lhs, test_root) << "' is not equal to '"
      << relative_path(rhs, test_root) << "'.";
    BOOST_CHECK_MESSAGE(result, s.str());
  }
}

static bool
less_target(const basic_target* lhs, const basic_target* rhs)
{
...
static bool
operator<(const build_node::source_t& lhs, const build_node::source_t& rhs)
{
...
{
public:
  test_msvc_solution(const project& source_project,
                     const location_t& output_location,
                     generation_mode::value mode)
    : msvc_solution(source_project, output_location, mode)
  {
    fill(id_, id_ + 16, 0);
  }
...
static int
compare_sources(const build_node::sources_t& lhs,
                const build_node::sources_t& rhs)
{
  for (build_node::sources_t::const_iterator i = lhs.begin(),
                                             i_r = rhs.begin(),
                                             last = lhs.end();
       i != last;
       ++i, ++i_r) {
    ...
    location_t rhs_id =
      i_r->source_target_->location() / i_r->source_target_->name();
    lhs_id.normalize();
    rhs_id.normalize();
...
static int
compare_products(const build_node::targets_t& lhs,
                 const build_node::targets_t& rhs)
{
  for (build_node::targets_t::const_iterator i = lhs.begin(),
                                             i_r = rhs.begin(),
                                             last = lhs.end();
       i != last;
       ++i, ++i_r) {
...
static bool
less_node(const boost::intrusive_ptr<build_node>& lhs,
          const boost::intrusive_ptr<build_node>& rhs);

static int
compare_dependencies(const build_node::nodes_t& lhs,
                     const build_node::nodes_t& rhs)
{
  for (build_node::nodes_t::const_iterator i = lhs.begin(),
                                           i_r = rhs.begin(),
                                           last = lhs.end();
       i != last;
       ++i, ++i_r) {
...
struct generator_tests : setuped_engine
{
  generator_tests()
    : p_(0)
    , generators_output_dir_name_(".hammer")
  {
  }
  ...
  void check_msvc_solution(const fs::path& test_data_path)
  {
    ...
    // gather files
    for (fs::recursive_directory_iterator last_etalon_files;
         i_etalon_files != last_etalon_files;
         ++i_etalon_files) {
      if (!is_directory(*i_etalon_files))
        etalon_files.insert(relative_path(*i_etalon_files, etalon_files_path));
    }

    for (fs::recursive_directory_iterator last_generated_files;
         i_generated_files != last_generated_files;
         ++i_generated_files) {
      ...
      if (!is_directory(*i_generated_files)) {
        ...
        if (ext == ".sln" || ext == ".vcproj")
          generated_files.insert(
            relative_path(*i_generated_files, generated_files_path));
      }
    }

    set<fs::path> unexpected_generated;
    set_difference(generated_files.begin(),
                   generated_files.end(),
                   etalon_files.begin(),
                   etalon_files.end(),
                   insert_iterator<set<fs::path>>(unexpected_generated,
                                                  unexpected_generated.end()));

    for (set<fs::path>::const_iterator i = unexpected_generated.begin(),
                                       last = unexpected_generated.end();
         i != last;
         ++i) {
      ostringstream s;
    ...
    set_difference(generated_files.begin(),
                   generated_files.end(),
                   unexpected_generated.begin(),
                   unexpected_generated.end(),
                   insert_iterator<set<fs::path>>(tmp, tmp.end()));
    tmp.swap(generated_files);

    // checking
    set<fs::path>::const_iterator c_etalon_files = etalon_files.begin(),
                                  c_last_etalon_files = etalon_files.end(),
                                  c_generated_files = generated_files.begin(),
                                  c_last_generated_files =
                                    generated_files.end();
  ...
  {
    for (build_node::nodes_t::iterator i = nodes.begin(), last = nodes.end();
         i != last;
         ++i)
      sort_node(**i);
  ...
  void run_generators(const fs::path& test_data_path,
                      msvc_solution::generation_mode::value mode =
                        msvc_solution::generation_mode::NON_LOCAL)
  {
    msvc_solution_.reset(new test_msvc_solution(
      *p_, test_data_path / generators_output_dir_name_, mode));
    for (vector<basic_target*>::iterator i = itargets_.begin(),
                                         last = itargets_.end();
         i != last;
         ++i) {
...
static void
test_function(const fs::path& test_data_path)
{
...
void
init_generators_tests(const fs::path& test_data_path)
{
  test_suite* ts = BOOST_TEST_SUITE("generators");
  for (fs::directory_iterator i(test_data_path / "generator_tests");
       i != fs::directory_iterator();
       ++i)
    if (i->path().filename() != ".svn")
      ts->add(make_test_case(boost::bind(&test_function, i->path()),
