...
warehouse_test::warehouse_test(const bool remove_packages,
  ...
  , installed_packages_path_(test_data_path /
                             "warehouse_tests/installed_packages")
  ...
  , libs_path_(test_data_path / "warehouse_tests/libs")
{
  ...
  warehouse_.reset(new warehouse_impl(
    "test", "file://" + packages_path_.string(), installed_packages_path_));
}

void
warehouse_test::add_all_libs()
{
  warehouse_->add_to_packages(
    engine_.load_project(libs_path_ / "lib1/1.0/build"));
  warehouse_->add_to_packages(
    engine_.load_project(libs_path_ / "lib1/2.0/build"));
  warehouse_->add_to_packages(
    engine_.load_project(libs_path_ / "lib2/1.0/build"));
  warehouse_->add_to_packages(
    engine_.load_project(libs_path_ / "lib2/2.0/build"));
}

void
warehouse_test::setup_engine_warehouse()
{
  const string script =
    (boost::format("setup-warehouse test : \"file://%1%\" : %2% ;") %
     packages_path_.string() % installed_packages_path_)
      .str();
...
static void
test_function_phase_1(const fs::path& test_data_path)
{
  ...
  const vector<warehouse::package_info> packages =
    env.engine_.warehouse().get_unresoved_targets_info(
      env.engine_, find_all_warehouse_unresolved_targets(instantiated_targets));

  null_warehouse_download_and_install notifier;
  BOOST_REQUIRE_NO_THROW(env.engine_.warehouse().download_and_install(
    env.engine_, packages, notifier));
}

static void
test_function_phase_2(const fs::path& test_data_path)
{
  // Phase 2
...
static void
test_step(const fs::path& test_data_path, const unsigned step)
{
...
add_steps(const fs::path& test_case_path)
{
  ...
  while (is_directory(step_path)) {
    ts->add(make_test_case(boost::bind(&test_step, step_path, step),
                           to_string(step)));
...
void
init_warehouse_auto_tests(const fs::path& test_data_path)
{
  using namespace boost::unit_test;

  test_suite* ts = BOOST_TEST_SUITE("warehouse");
  for (fs::directory_iterator i(test_data_path / "warehouse_tests/tests");
