...
{
  hammer::types::register_standart_types(engine_.get_type_registry(),
                                         engine_.feature_registry());
  if (install_toolsets) {
    engine_.toolset_manager().add_toolset(
      std::unique_ptr<hammer::toolset>(new hammer::msvc_toolset()));
    hammer::location_t fake_location("fake_msvc_location");
    engine_.toolset_manager().init_toolset(
      engine_, "msvc", "8.0", &fake_location);
  }
...
{
  engine_.call_resolver().insert(
    "use-toolset",
    boost::function<void(project*, string&, string&, string*)>(
      boost::bind(use_toolset_rule, _1, boost::ref(engine_), _2, _3, _4)));

  hammer::types::register_standart_types(engine_.get_type_registry(),
                                         engine_.feature_registry());
  install_toolsets();

  // Simplest way is to specify /home/username/user-config.ham as toolset
  // configuring script
  const char* toolset_setup_script = getenv("HAMMER_TOOLSET_SETUP_SCRIPT");
  BOOST_REQUIRE_MESSAGE(toolset_setup_script,
                        "You MUST set HAMMER_TOOLSET_SETUP_SCRIPT environment "
                        "variable to script with toolset configuring "
                        "instructions");
...
void
complete_build_tests_environment::install_toolsets()
{
...
void
complete_build_tests_environment::run_test(
  ...
  std::function<void(const boost::filesystem::path& working_dir)> test_function)
{
  ...
  for (fs::directory_iterator i(test_data_path); i != fs::directory_iterator();
       ++i) {
