...
struct enviroment
{
  enviroment()
  {
...

    {
      ...
      attrs.free = 1;
      fr_.add_feature_def(
        "optimization", list_of<string>("off")("speed")("size"), attrs);
    }
...

    {
      ...
      debug_components->join("optimization", "off").join("debug-symbols", "on");

      feature_set* release_components = fr_.make_set();
      release_components->join("optimization", "on")
        .join("debug-symbols", "off");

      feature_def& def = fr_.add_feature_def(
        "variant", list_of<string>("debug")("release"), attrs);
