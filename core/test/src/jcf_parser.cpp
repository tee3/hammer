...
displayRecognitionError(pANTLR3_BASE_RECOGNIZER recognizer,
                        pANTLR3_UINT8* tokenNames)
{
  jcf_parser_context* ctx = static_cast<jcf_parser_context*>(
...
jcf_parser::jcf_parser()
  : impl_(new impl_t)
{
}

bool
jcf_parser::parse(const boost::filesystem::path& file_name)
{
  if (!exists(file_name))
...
bool
jcf_parser::parse(const char* file_name)
{
  ...
  impl_->tstream_ = antlr3CommonTokenStreamSourceNew(
    ANTLR3_SIZE_HINT, TOKENSOURCE(impl_->lexer_));
...
void
jcf_parser::walk(const vector<hammer::basic_target*>& targets,
                 hammer::engine* e)
{
  ...
  jcf_walker->jcf_file(jcf_walker,
                       const_cast<vector<hammer::basic_target*>*>(&targets));
