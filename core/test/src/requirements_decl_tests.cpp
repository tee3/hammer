...
struct environment
{
  environment()
  {
...

    {
      feature_attributes attrs = { 0 };
      attrs.propagated = 1;
      feature_def& toolset_def =
        fr_.add_feature_def("toolset", { "gcc", "msvc" }, attrs);
      subfeature_def& toolset_version_def =
        toolset_def.add_subfeature("version");
...
requirements_decl
make_requirements_decl(feature_registry& fr, const string& toolset)
{
