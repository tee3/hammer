...
void*
get_target(const char* id, void* t, int is_top)
{
  if (t == 0)
    return 0;
  ...
  if (is_top) {
    const vector<basic_target*>* targets =
      static_cast<const vector<basic_target*>*>(t);
    ...
    if (mt) {
      for (vector<basic_target*>::const_iterator i = mt->sources().begin(),
                                                 last = mt->sources().end();
           i != last;
           ++i) {
        if ((**i).name() == id)
...
void
check_type(void* e, void* t, const char* type_id)
{
  if (!t)
    return;
  ...
  if (!bt->type().equal_or_derived_from(*et))
    BOOST_CHECK_MESSAGE(false,
                        format("Expected type '%s' but got '%s'. Target '%s'") %
                          type_id % bt->type().tag().name() % bt->name());
}
...
void
check_feature(void* e,
              void* t,
              void* features,
              const pANTLR3_COMMON_TOKEN name_token,
              const pANTLR3_COMMON_TOKEN value_token)
{
  const char* name =
    reinterpret_cast<const char*>(name_token->getText(name_token)->chars);
  const char* value =
    reinterpret_cast<const char*>(name_token->getText(value_token)->chars);
  ...
  const feature_def* local_def =
    bt->get_project()->local_feature_registry().find_def(name);
  ...
  if (fd.attributes().path) {
    ...
    if (f == fs->end()) {
      BOOST_CHECK_MESSAGE(
        false,
        format("%s(%d) : Expected feature '%s' with value '%s' was not found") %
          reinterpret_cast<const char*>(name_token->input->fileName->chars) %
          name_token->line % name % value);
      return;
    }
    ...
    if (p != expected_path)
      BOOST_CHECK_MESSAGE(
        false,
        format("%s(%d) : Expected feature '%s' with "
               "value '%s' but found value '%s'") %
          reinterpret_cast<const char*>(name_token->input->fileName->chars) %
          name_token->line % name % value % p);
    return;
  }
  ...
  if (!fs->find(name, value)) {
    BOOST_CHECK_MESSAGE(
      false,
      format("%s(%d) : Expected feature '%s' with value '%s' for target "
             "'%s\\\\%s' was not found") %
        reinterpret_cast<const char*>(name_token->input->fileName->chars) %
        name_token->line % name % value % bt->get_main_target()->location() %
        bt->name());
    return;
  }
}
                  ...
                  const char* value)
{
  ...
  if (fs->find(name, value))
    BOOST_CHECK_MESSAGE(
      false,
      format("Unexpected feature '%s' with value '%s' found in properties") %
        name % value);
}

void
check_location(void* t, const char* location)
{
  const basic_target* bt = static_cast<const basic_target*>(t);
  if (const main_target* mt = dynamic_cast<const main_target*>(bt)) {
    if (mt->location() != location)
      BOOST_CHECK_MESSAGE(false,
                          format("Expected location '%s' but got '%s'") %
                            location % mt->location());
  } else
    BOOST_CHECK_MESSAGE(
      false, format("Target '%s' is not a main target") % bt->name());
}

void
check_number_of_sources(void* t, const char* number)
{
  const basic_target* bt = static_cast<const basic_target*>(t);
  const main_target* mt = dynamic_cast<const main_target*>(bt);
  BOOST_REQUIRE(mt);
