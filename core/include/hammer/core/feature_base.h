...
public:
  struct path_data
  {
    ...
    bool operator!=(const path_data& rhs) const
    {
  ...
  {
    source_decl source_;
    bool operator<(const dependency_data& rhs) const
    {
  ...
  struct generated_data
  {
    ...
    bool operator<(const generated_data& rhs) const
    {
      return target_ < rhs.target_;
    }
    bool operator!=(const generated_data& rhs) const
  ...
  const dependency_data& get_dependency_data() const
  {
    return dependency_data_;
  }
  // dependency data ALWAYS has target relative to which source_decl.target_path
  // was calculated
