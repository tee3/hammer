...
public:
  fs_argument_writer(const std::string& name, feature_registry& fr)
    ...
    , feature_registry_(&fr)
  {
  }

  fs_argument_writer& add(const feature_set* pattern,
                          const std::string& what_write);
  fs_argument_writer& add(const std::string& pattern,
                          const std::string& what_write);
  virtual fs_argument_writer* clone() const;

private:
  typedef std::vector<
    std::pair<const feature_set* /*pattern*/, std::string /*what write*/>>
    patterns_t;
