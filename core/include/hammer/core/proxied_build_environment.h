...
{
public:
  proxied_build_environment(const build_environment& env)
    : env_(env)
  {
  }
  ...
  bool run_shell_commands(std::ostream& captured_output_stream,
                          ...
                          const location_t& working_dir) const override
  {
    return env_.run_shell_commands(
  ...
  void remove(const location_t& p) const override { env_.remove(p); }
  ...
  std::unique_ptr<std::ostream> create_output_file(
    const char* filename,
  ...
  std::ostream& output_stream() const override { return env_.output_stream(); }

  std::ostream& error_stream() const override { return env_.error_stream(); }

  const location_t* cache_directory() const { return env_.cache_directory(); }
