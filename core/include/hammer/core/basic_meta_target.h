...
typedef std::vector<
  std::pair<const basic_meta_target* /* target */,
            const feature_set* /* requested build properties*/>>
  meta_targets_t;
...
public:
  ...
  const requirements_decl& usage_requirements() const
  {
    return usage_requirements_;
  }
  ...
  void usage_requirements(const requirements_decl& r)
  {
    usage_requirements_ = r;
  }
