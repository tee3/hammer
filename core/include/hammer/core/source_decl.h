...
// FIXME: source_decl should have method resolve_type for resolving type of
// source
...
public:
  ...
  void target_path(const std::string& v, const target_type* t)
  {
    target_path_ = v;
    type_ = t;
  }
  ...
  bool target_path_is_global() const
  {
    return !target_path_.empty() && *target_path_.begin() == '/';
  }
  ...
  bool operator<(const source_decl& rhs) const
  {
    if (target_path_ < rhs.target_path_)
      return true;
    else if (target_path_ == rhs.target_path_)
      return false;
    else if (target_name_ < rhs.target_name_)
      return true;
    else
      return false;
  }
  ...
  bool operator==(const source_decl& rhs) const
  {
    return target_path_ == rhs.target_path_ && target_name_ == rhs.target_name_;
  }
