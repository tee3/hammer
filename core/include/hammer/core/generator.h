...
public:
  ...
  {
    consumable_type(const target_type& t,
                    unsigned int f = 0,
                    const feature_set* fs = 0)
      : type_(&t)
      , flags_(f)
      , features_(fs)
    {
  ...
  {
    produced_type(const target_type& t, bool need_tag = false)
      : type_(&t)
      , need_tag_(need_tag)
    {
    }
  ...
  virtual build_nodes_t construct(const target_type& type_to_construct,
  ...
  bool include_composite_generators() const
  {
    return include_composite_generators_;
  }
  void include_composite_generators(bool v)
  {
    include_composite_generators_ = v;
  }

  template<typename T>
  void action(std::unique_ptr<T> a)
  {
    action_ = std::move(a);
  }
...
private:
  ...
  bool include_composite_generators_; // include composite generators while
                                      // searching for sources indirect
                                      // transformations
};

// we must not change order in nodes, because if that we use multi_index
// because of gcc library ordering we leave only last ocurrancies of some
// library.
