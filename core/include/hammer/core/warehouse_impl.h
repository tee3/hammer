...
public:
  ...
  boost::shared_ptr<project> load_project(
    engine& e,
    const location_t& project_path) override;

  std::vector<package_info> get_unresoved_targets_info(
    engine& e,
...
private:
  ...
  typedef boost::unordered_map<std::string, dependency_t>
    unresolved_dependencies_t;
  ...
  static packages_t::iterator find_package(packages_t& packages,
  ...
  void install_package(const package_t& p,
                       const boost::filesystem::path& working_dir);
  void update_package(engine& e, package_t& package_to_update);
  void remove_package(const package_t& package_to_remove);

  static packages_t load_packages(const boost::filesystem::path& filepath);
  ...
  static void write_packages(const location_t& packages_db_path,
                             const packages_t& packages);

  static std::vector<dependency_t> gather_dependencies(const project& p);
};
}
