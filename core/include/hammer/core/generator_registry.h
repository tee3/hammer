...
{
public:
  typedef std::unordered_map<std::string, std::unique_ptr<generator>>
    generators_t;
  typedef std::vector<
    std::pair<const generator*, const target_type* /* type_to_construct */>>
    viable_generators_t;

  void insert(std::unique_ptr<generator> g);
  build_nodes_t construct(const main_target* mt) const;

  viable_generators_t find_viable_generators(
    const target_type& t,
