...
public:
  ...
  typedef boost::ptr_multimap<const std::string /* target name */,
                              basic_meta_target>
    targets_t;
  ...
  feature_registry& local_feature_registry() const
  {
    return local_feature_registry_;
  }
  bool is_root() const { return is_root_; }
  void set_root(bool v) { is_root_ = v; }
  // FIXME: both functions are hacks. We should remove them after migrate to new
  // parser
  ...
  // select targets in project that satisfied build_request. Can return empty
  // list.
  selected_targets_t select_best_alternative(
    const feature_set& build_request) const;

  // choose best alternative for target_name satisfied build_request
  selected_target select_best_alternative(
