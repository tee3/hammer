...
public:
  ...
  const feature_def& definition() const
  {
    return *static_cast<const feature_def*>(definition_);
  }
  ...
  // <toolset>gcc will be equal only to <toolset>gcc - full match, including
  // subfeatures
  bool operator==(const feature& rhs) const;

  // if this == <toolset>gcc-6 then it will contain f == <toolset>gcc AND f ==
  // <toolset>gcc-6, partial match
