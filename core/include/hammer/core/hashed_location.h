...
public:
  ...
  {
    return static_cast<const location_t&>(*this);
  }
  ...
  bool operator==(const hashed_location& rhs) const
  {
    return hash_ == rhs.hash_ && string() == rhs.string();
  }
...
inline std::size_t
hash_value(const hashed_location& hl)
{
  return hl.hash();
}
}
