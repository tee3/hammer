...
public:
  ...
  // differs from find in terms of equality operator. It uses
  // feature::contains(), so
  ...
  const_iterator find(const_iterator from,
                      const char* name) const; // find next occurrence
  const_iterator find(const_iterator from, const std::string& name) const
  {
    return find(from, name.c_str());
  }
...
void
dump_for_hash(std::ostream& s, const feature_set& fs, bool dump_all = false);
std::string
dump_for_hash(const feature_set& fs, bool dump_all = false);
