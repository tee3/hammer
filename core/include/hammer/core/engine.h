...
public:
  ...
  const hammer::feature_registry& feature_registry() const
  {
  ...
  const hammer::scanner_manager& scanner_manager() const
  {
    return *scanner_manager_;
  }
  hammer::warehouse& warehouse() { return *warehouse_; }

  hammer::output_location_strategy& output_location_strategy()
  {
    return *output_location_strategy_;
  }
  void output_location_strategy(
...
private:
  ...
  typedef boost::unordered_map<location_t,
                               ...
                               boost::hash<location_t>,
                               location_equal_to>
    reversed_global_project_links_t;

  struct project_alias_node
  {
    typedef std::vector<project_alias_data> aliases_data_t;

    // mapping associated with this node. One project symlink path such as
    // /foo/bar
  ...
  {
    resolved_project_symlink_t(
      const location_t& tail,
    ...
    // Unresolved part. bar is a tail_, if /foo/bar was requested and foo was
    // founded.
    location_t tail_;
  ...
  loaded_projects_t try_load_project(const location_t& tail_path,
  ...
  void searched_shared_lib_rule(project* p,
                                ...
                                requirements_decl* usage_requirements);
  void searched_static_lib_rule(project* p,
                                std::string& name,
                                sources_decl* sources,
                                std::string& lib_name,
                                requirements_decl* requirements,
                                requirements_decl* usage_requirements);
  void prebuilt_lib_rule(project* p,
                         std::string& name,
                         sources_decl* sources,
  ...
  void feature_feature_rule(project* p,
                            std::vector<std::string>& name,
...
public:
  ...
  explicit loaded_projects_t(project* v)
    : projects_(1, v)
  {
  }
  ...
  {
    projects_.insert(
      projects_.end(), rhs.projects_.begin(), rhs.projects_.end());
    return *this;
  }
  project::selected_targets_t select_best_alternative(
    const feature_set& build_request) const;
  project::selected_target select_best_alternative(
