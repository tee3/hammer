...
static void
project_rule(const parscore::identifier& id,
...
static void
lib_rule(const parscore::identifier& id,
...
static void
exe_rule(const parscore::identifier& id,
         const hammer::sources_decl* sources,
         const hammer::requirements_decl* requirements,
         const hammer::feature_set* default_build,
         const hammer::requirements_decl* usage_requirements)
...
static void
glob_rule(const hammer::path_like_seq& pattern)
{
}

static void
rglob_rule(const hammer::path_like_seq& pattern)
{
}
...
void
checked_diagnostic::report(const char* formated_message)
{
  boost::cmatch m;
  if (regex_search(
        formated_message, m, boost::regex(".+?\\((\\d+)\\) : (\\w+): (.*)"))) {
    // m[1] line number
    // m[2] message type
    // m[3] message
...
void
checked_diagnostic::report_unreported_diagnostics() const
{
  ...
  for (const auto& i : expected_diags_)
    if (reported_lines_.find(i.first) == reported_lines_.end())
      BOOST_CHECK_MESSAGE(
        false, string("Diagnostic '") + i.second.first + "' was not reported");
    else
...
static expected_diags_t
extract_expected_diags(const fs::path& hamfile)
{
...
void
test_function(const fs::path& hamfile)
{
  ...
  {
    vector<parscore::identifier> arg_names;
    arg_names += "target-name", "sources", "requirements", "default-build",
      "usage-requirements";
    rule_manager.add_target(
      "lib",
      boost::function<void(const parscore::identifier&,
  ...
  fs::ofstream f(hamfile.branch_path() /
                 (hamfile.filename().string() + ".ast"));
...
init_unit_test_suite(int argc, char* argv[])
{
  ...
  for (fs::directory_iterator i(test_data_path); i != fs::directory_iterator();
    ...
    if (i->path().filename() != ".svn" && extension(i->path()) == ".ham") {
      framework::master_test_suite().add(make_test_case(
        boost::bind(&test_function, i->path()), basename(i->path())));
    }
