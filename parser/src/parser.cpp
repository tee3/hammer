...
parser::parse_impl()
{
  ...
  ctx->input_ =
    antlr3AsciiFileStreamNew((pANTLR3_UINT8)hamfile_.string().c_str());
  ctx->lexer_ = hammerLexerNew(ctx->input_);
  ctx->tstream_ = antlr3CommonTokenStreamSourceNew(ANTLR3_SIZE_HINT,
                                                   TOKENSOURCE(ctx->lexer_));
  ...
  nodes =
    antlr3CommonTreeNodeStreamNewTree(ctx->langAST_.tree, ANTLR3_SIZE_HINT);
