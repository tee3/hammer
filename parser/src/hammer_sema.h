 ...
 * function) via
 * a parser context typedef phammer_sema, which is returned from a call to
 * hammer_semaNew().
 *
 ...
 *  - const hammer::ast::expression*
 * phammer_sema->target_def_or_rule_call(phammer_sema)
 *  - void      phammer_sema->arguments(phammer_sema)
 *  - const hammer::ast::expression*      phammer_sema->argument(phammer_sema)
 *  - const hammer::ast::expression*
 * phammer_sema->named_expression(phammer_sema)
 *  - const hammer::ast::expression*      phammer_sema->expression(phammer_sema)
 *  - const hammer::ast::requirement_set*
 * phammer_sema->requirement_set(phammer_sema)
 *  - const hammer::ast::requirement* phammer_sema->requirement(phammer_sema)
 *  - const hammer::ast::requirement*
 * phammer_sema->conditional_requirement(phammer_sema)
 *  - const hammer::ast::requirement*
 * phammer_sema->simple_requirement(phammer_sema)
 ...
 *  - const hammer::ast::requirement_set*
 * phammer_sema->target_ref_requirements(phammer_sema)
 *  - const hammer::ast::path_like_seq*
 * phammer_sema->path_like_seq(phammer_sema)
 ...
 */
  ...
  void (*statement)(struct hammer_sema_Ctx_struct* ctx,
                    hammer::ast::statements_t* statements);
  const hammer::ast::expression* (*target_def_or_rule_call)(
    struct hammer_sema_Ctx_struct* ctx);
  void (*arguments)(struct hammer_sema_Ctx_struct* ctx,
                    hammer::ast::expressions_t* args);
  const hammer::ast::expression* (*argument)(
    struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::expression* (*named_expression)(
    struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::expression* (*expression)(
    struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::requirement_set* (*requirement_set)(
    struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::requirement* (*requirement)(
    struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::requirement* (*conditional_requirement)(
    struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::requirement* (*simple_requirement)(
    struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::feature* (*feature)(struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::expression* (*feature_value)(
    struct hammer_sema_Ctx_struct* ctx);
  void (*list_of)(struct hammer_sema_Ctx_struct* ctx,
                  hammer::ast::expressions_t* args);
  const hammer::ast::expression* (*list_of_impl)(
    struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::expression* (*target_ref)(
    struct hammer_sema_Ctx_struct* ctx);
  void (*public_tag)(struct hammer_sema_Ctx_struct* ctx,
                     hammer::parscore::source_location* tag_loc);
  void (*target_ref_name)(struct hammer_sema_Ctx_struct* ctx,
                          hammer::parscore::identifier* name);
  const hammer::ast::requirement_set* (*target_ref_requirements)(
    struct hammer_sema_Ctx_struct* ctx);
  const hammer::ast::path_like_seq* (*path_like_seq)(
    struct hammer_sema_Ctx_struct* ctx);
  void (*path_like_seq_impl)(
    struct hammer_sema_Ctx_struct* ctx,
