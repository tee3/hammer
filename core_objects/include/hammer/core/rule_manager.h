...
public:
  ...
  static rule_argument_type::value type(const hammer::parscore::identifier*)
  {
    return IDENTIFIER;
  }

  static rule_argument_type::value type(const feature_set*)
  {
    return FEATURE_SET;
  }

  static rule_argument_type::value type(const sources_decl*)
  {
    return SOURCES_DECL;
  }

  static rule_argument_type::value type(const requirements_decl*)
  {
    return REQUIREMENTS_DECL;
  }

  static rule_argument_type::value type(const path_like_seq*)
  {
    return PATH_LIKE_SEQ;
  }
};
...
make_one_arg(const parscore::identifier& arg_name)
{
  ...
  return rule_argument(
    rule_argument_type::type(static_cast<const pure_arg_t*>(NULL)),
    arg_name,
    boost::mpl::bool_<boost::is_pointer<T>::value>());
}

template<>
inline rule_argument
make_one_arg<void>(const parscore::identifier& arg_name)
{
...
template<typename T>
void
push_arg_impl(std::vector<rule_argument>* args,
...
make_args(const std::vector<parscore::identifier>& arg_names)
{
  ...
  push_arg_impl<T>(
    &result,
    arg_names,
    boost::mpl::int_<boost::function_types::function_arity<T>::value - 1>());
  return result;
}
}
                ...
                bool is_target)
  {
    ...
    if (arg_names.size() != boost::function_types::function_arity<T>::value)
      throw std::runtime_error(
        "[hammer.rule_manager] Not enought argument names");

    rule_declaration decl(
      id,
    ...
    if (!rules_.insert(std::make_pair(id, decl)).second)
      throw std::runtime_error("[hammer.rule_manager] Rule '" + id.to_string() +
                               "' already added");
  }
};
}
