...
BOOST_AUTO_TEST_CASE(exe_rule_test)
{
  typedef boost::function<void(
    const identifier&, const sources_decl&, const feature_set*)>
    func_type;
  ...
  BOOST_REQUIRE(i != m.end());
  BOOST_REQUIRE_THROW(
    m.add_target(rule_id, func_type(exe_rule), rule_arg_names), std::exception);
